...
public:
  DEPFETProducerTCP(const std::string &name, const std::string &runcontrol)
      : eudaq::Producer(name, runcontrol), done(false), host_is_set(false),
        running(false), firstevent(false) {
    //
  }
  ...
  virtual void OnReset() { SetStatus(eudaq::Status::LVL_OK, "Reset"); }
  void Process() {
    eudaq::Timer timer;
    ...
    do { //--- modules of one event loop
      ...
      eudaq::Timer timer2;
      int rc =
          tcp_event_get(&data_host[0], buffer, &lenevent, &Nmod, &Kmod, &itrg);
      if (itrg % 100 == 0)
        std::cout << "##DEBUG## tcp_event_get " << timer2.mSeconds() << "ms"
                  << std::endl;
      if (rc < 0)
      ...
      if (itrg == BORE_TRIGGERID || itrg == EORE_TRIGGERID ||
          itrg < itrg_old /*|| evt_type != 2*/) {
        std::cout << "Received: Mod " << (Kmod + 1) << " of " << Nmod
                  << ", id=" << evtModID << ", EvType=" << evt_type
                  << ", DevType=" << dev_type << ", NData=" << lenevent << " ("
                  << len2 << ") "
                  << ", TrigID=" << itrg << " (" << buffer[1] << ")"
                  << std::endl;
      }
      ...
      if (!ev) {
        ...
        ev = make_shared<eudaq::RawDataEvent>(
            "DEPFET", m_run,
            m_evt); // -- fsv:: send local counter instead TLU number
      }
      ev->AddBlock(id++, buffer, lenevent * 4);

    } while (Kmod != (Nmod - 1));
    if (itrg % 100 == 0)
      std::cout << "##DEBUG## Reading took " << timer.mSeconds() << "ms"
                << std::endl;
    timer.Restart();
    ...
    SendEvent(*ev);
    if (itrg % 100 == 0)
      std::cout << "##DEBUG## Sending took " << timer.mSeconds() << "ms"
                << std::endl;
  }
  bool done;
