...
//                      S E N D
//---------------------------------------------------------------------------------------------------------------
int tcp_event_snd(unsigned int *DATA, int lenDATA, int n, int k,
                  unsigned int evtHDR, unsigned int TriggerID) {
...
//                      G E T
//---------------------------------------------------------------------------------------------------------------
int tcp_event_get(char *HOST2, unsigned int *DATA, int *lenDATA,
                  int *Nr_Modules, int *ModuleID, unsigned int *TriggerID) {
  static char hostname[100];
  static int sd;
  static struct sockaddr_in pin;
  struct hostent *hp;
  ...
  if (TCP_FLAG == 0) {
    ...
    printf("TCPclient:: t1=%d  t0=%d t1-t0=%d wait %d sec...\n", time1, time0,
           time1 - time0, T_WAIT - (time1 - time0));
    if ((time1 - time0) > T_WAIT) {
      time0 = time1;
      printf("TCPclient:: %d RE-try to connect to %s\n", time1 - time0,
             hostname);
  ...
  *ModuleID = HEADER[6];
  ...
  // printf("HEADER OK=%#X, Nr_MOD=%d, ModID=%d,
  ...
  printf("OK, TCP recv=%d  nleft=%d Trg=%d(%d) Mod=%d siz=%d\n", nread, nleft,
         evtTrigID, *TriggerID, evtModID, evtSize);
