...
int STREQ(char *s1, const char *s2) {
  if (strncasecmp(s1, s2, strlen(s2)))
    return 0;
  else
    return 1;
}
...
int run_control(struct HOST *host) {
  ...
  while (1) {
    ...
    if (!strncmp(Buffer, "CMD_EVB_TEST", 7)) {
      printf("EVB run_control():: \n");
    } else {
      if (field_cmd(Buffer) < 0)
        break;
    }
  }
  close(cmd_socket);
...
int TCP_CONNECT(char *HOST, int PORT) {
  ...
  if ((Socket = socket(AF_INET, SOCK_STREAM, 0)) < 0) {
    printf("TCP_CONNECT():: Can't open datagram socket");
    exit(106);
  }
  ...
  if ((ret = send(Socket, (void *)INFO, strlen(INFO), 0)) !=
      (int)strlen(INFO)) {
...
int cmd_send(const std::string &command) {
  ...
  while (cmd_socket <= 0) {
    printf("cmd_send():: Wait for connection sock=%d\n", cmd_socket);
    sleep(1);
  }

  if ((ret = send(cmd_socket, (void *)command.c_str(), command.length(), 0)) !=
      (int)command.length()) {
    printf("Error Send INFO ret=%d of (%d)\n", ret, (int)command.length());
    perror("send to RC");
  }
