...
class CPixelmanProducerMFCDlg : public CDialog {
  DECLARE_DYNAMIC(CPixelmanProducerMFCDlg) // Adds the ability to access
                                           // run-time
...
public:
   ...
   *	Note: This function is thread safe because it only uses thread safe
   *functions
   * (I hope that mpxCtrlTriggerType() is tread safe...)
   */
  int mpxCheckForTrigger();

  // There might be several plugins available, so we have to define which
  // module/chip is readout
