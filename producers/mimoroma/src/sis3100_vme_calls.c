...
int vmesysreset(int p) {
  if (s3100_control_write(p, 0x100 /*offset*/, 0x2 /*data*/) != 0x0)
    return -1;
  usleep(500000); /* 500ms (min. 200ms) */
  if (s3100_control_write(p, 0x100 /*offset*/, 0x20000 /*data*/) != 0x0)
    return -1;
  return 0;
}
...
int vme_A24DMA_D32_write(int p, u_int32_t vme_adr, u_int32_t *vme_data,
  ...
  block_req.addr = vme_adr;
  block_req.data = (u_int8_t *)vme_data;
  if (ioctl(p, SIS3100_VME_BLOCK_WRITE, &block_req) < 0)
    return -1; /* NEW */
