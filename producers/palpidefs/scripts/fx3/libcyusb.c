...
int cyusb_open(void) {
  ...
  if (fd1 < 0) {
    ...
    close(fd1);
    parse_configfile(); /* Parses the file and stores critical information
                           inside exported data structures */
  }
...
int cyusb_set_interface_alt_setting(cyusb_handle *h, int interface,
                                    int altsetting) {
...
int cyusb_reset_device(cyusb_handle *h) { return (libusb_reset_device(h)); }
...
int cyusb_get_active_config_descriptor(
    cyusb_handle *h, struct libusb_config_descriptor **config) {
  cyusb_device *tdev = libusb_get_device(h);
  return (libusb_get_active_config_descriptor(tdev, config));
}

int cyusb_get_config_descriptor(cyusb_handle *h, unsigned char config_index,
...
int cyusb_get_config_descriptor_by_value(
    cyusb_handle *h, unsigned char bConfigurationValue,
...
int cyusb_get_string_descriptor_ascii(cyusb_handle *h, unsigned char index,
...
int cyusb_get_descriptor(cyusb_handle *h, unsigned char desc_type,
                         unsigned char desc_index, unsigned char *data,
                         int len) {
  return (libusb_get_descriptor(h, desc_type, desc_index, data, len));
}

int cyusb_get_string_descriptor(cyusb_handle *h, unsigned char desc_index,
                                unsigned short langid, unsigned char *data,
                                int len) {
...
int cyusb_control_read(cyusb_handle *h, unsigned char bmRequestType,
                       unsigned char bRequest, unsigned short wValue,
                       unsigned short wIndex, unsigned char *data,
                       unsigned short wLength, unsigned int timeout) {
...
int cyusb_control_write(cyusb_handle *h, unsigned char bmRequestType,
                        unsigned char bRequest, unsigned short wValue,
                        unsigned short wIndex, unsigned char *data,
                        unsigned short wLength, unsigned int timeout) {
...
int cyusb_download_fx2(cyusb_handle *h, char *filename,
...
static void control_transfer(cyusb_handle *h, unsigned int address,
                             unsigned char *dbuf, int len) {
  int j;
  int r;
  int b;
  unsigned int *pint;
