...
#define GET_MSW(v)                                                             \
  ((unsigned short)((v) >> 16)) // Get Most Significant Word part of an integer.
...
static int fx3_ram_write(cyusb_handle *h, unsigned char *buf,
...
static int check_fx3_flashprog(cyusb_handle *handle) {
  ...
  r = cyusb_control_transfer(handle, 0xC0, 0xB0, 0, 0, (unsigned char *)local,
                             8, VENDORCMD_TIMEOUT);
...
static int fx3_i2c_write(cyusb_handle *h, unsigned char *buf, int devAddr,
                         int start, int len) {
  int r = 0;
  int index = start;
...
static int fx3_i2c_read_verify(cyusb_handle *h, unsigned char *expData,
                               int devAddr, int len) {
  int r = 0;
  int index = 0;
  unsigned short address = 0;
...
void print_usage_info(const char *arg0) {
  ...
  printf("\t%s -t <target> -i <img filename>: Program firmware binary from "
         "<img filename>\n",
         arg0);
  printf("\t\tto <target>\n");
