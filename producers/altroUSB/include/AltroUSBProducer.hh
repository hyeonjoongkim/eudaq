...
public:
  ...
  //     *  the conversion, which is faster (memory does not have to be
  //     allocated and
  //     *  released).
  //     */
   ...
   */
protected:
  /** Configure the hardware. The configuration files are read in using Ulf's
   * routines.
  ...
  bool m_configured; // no need for a mutex, only accessed by the communication
                     // thread
  /// The run number
  unsigned m_run;
  ...
  unsigned int
      m_block_size; ///< size of the u2fdata memory block (number of bytes)
  unsigned char *
      m_data_block; ///< the memory block where the u2f dumps the data it reads
