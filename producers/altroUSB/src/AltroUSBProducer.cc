...
AltroUSBProducer::AltroUSBProducer(const std::string &name,
    ...
    : eudaq::Producer(name, runcontrol), m_runactive(false),
      m_configured(false), m_run(0), m_ev(0), m_daq_config(0), m_block_size(0),
      m_data_block(0) {
...
//    );
//
//    // a data block of unsigned char, in this the data is stored in little
//    endian
//    unsigned char *serialdatablock =  new unsigned
...
void AltroUSBProducer::OnConfigure(const eudaq::Configuration &param) {
  ...
  // allocate  4300800 = 4200 * 1024 bytes , that should always be enough for
  // one u2f
  if (m_data_block == 0) {
...
void AltroUSBProducer::OnStartRun(unsigned param) {
  ...
  // check that configuration has been run. This ensures that the hardware is
  // set up correctly
  // and the memory block is allocated.
  if (!m_configured) {
    EUDAQ_WARN(
        "Cannot start run, hardware is nor confiugred. Run configure first!");
  ...
  SendEvent(eudaq::RawDataEvent::BORE(
      "AltroUSB", param)); // send param instead of GetRunNumber
...
void AltroUSBProducer::OnStopRun() {
  ...
  SendEvent(
      eudaq::RawDataEvent::EORE("AltroUSB", GetRunNumber(), GetEventNumber()));
...
void AltroUSBProducer::OnStatus() {
  ...
  // they will obscure all other commands. Handle this in the communication
  // thread if necessary,
  // i. e. in this function.
  // For the time being: do nothing
}

void AltroUSBProducer::OnUnrecognised(const std::string &cmd,
...
void AltroUSBProducer::Exec() {
  ...
  // flag set if the run is to be finished, i.e. trigger is switched off, all
  // remaining events are read out
  ...
  while (!terminate) {
    ...
    switch (command) {
    case NONE:
      break; // no command, nothing to do
    case START_RUN:
    ...
    } else // run is active, read out next event
    {
      ...
      // read the altro in block of 1024 bytes. This is the minimal size, and
      // the size of a USB burst
      // Like this it is ensured that the data is shipped once the event is
      // finished, and is read
      // as soon as it is available
      unsigned int nbytesread = 0;
      for (unsigned int block_index = 0; block_index < m_block_size / 1024;
           block_index++) {
        ...
        std::cout << "Readings in mode " << acquisition_mode << " to address "
                  << std::hex << reinterpret_cast<void *>(m_data_block +
                                                          (block_index * 1024))
                  << std::dec << std::endl;
        ...
        if (osize >= 8) {
          std::cout << std::hex << "0x " << std::setfill('0') << std::setw(2)
                    << static_cast<unsigned int>(m_data_block[nbytesread - 8])
                    << std::setw(2)
                    << static_cast<unsigned int>(m_data_block[nbytesread - 7])
                    << std::setw(2)
                    << static_cast<unsigned int>(m_data_block[nbytesread - 6])
                    << std::setw(2)
                    << static_cast<unsigned int>(m_data_block[nbytesread - 5])
                    << std::setw(2)
                    << static_cast<unsigned int>(m_data_block[nbytesread - 4])
                    << std::setw(2)
                    << static_cast<unsigned int>(m_data_block[nbytesread - 3])
                    << std::setw(2)
                    << static_cast<unsigned int>(m_data_block[nbytesread - 2])
                    << std::setw(2)
                    << static_cast<unsigned int>(m_data_block[nbytesread - 1])
                    << std::dec << std::endl;
        }
        ...
        if (osize) // only do this check if the outsize is not 0
        {
          if ((m_data_block[nbytesread - 1] == 0xff) &&
              ...
              (m_data_block[nbytesread - 8] ==
               0xff)) { // end of event signature found
            std::cout << "End of event signature found" << std::endl;
            break;
          } else // number of bytes has to be 1024, otherwise there is something
                 // wrong
          {
            if (osize != 1024) {
      ...
      if (nbytesread) {
        eudaq::RawDataEvent event("AltroUSB", GetRunNumber(),
                                  GetIncreaseEventNumber());
      ...
      //	    {
      //		if (i%4==0)
      //		    std::cout << std::dec<<  std::endl << i/4  <<"\t 0x
      //" << std::hex << std::setfill('0');
      //
      //		std::cout << std::setw(2) << static_cast<unsigned
      //short>(m_data_block[i]) <<" ";
      //	    }
