...
int MIMTLU::Connect(char *IP, char *port) {
  ...
  // Connection
  status =
      connect(socketfd, host_info_list->ai_addr, host_info_list->ai_addrlen);
  if (status == -1) {
    std::cout << "connect error " << gai_strerror(status) << " "
              << host_info.ai_flags << endl;
    return -3;
  } else {
...
std::vector<mimtlu_event> MIMTLU::GetEvents() {
  ...
  // If no data arrives, the program will just wait here until some data
  // arrives.
  if (bytes_recieved == 0)
