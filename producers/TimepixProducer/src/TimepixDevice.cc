...
// char configName[256] =
// "/home/lcd/CLIC_Testbeam_August2013/TimepixAssemblies_Data/C04-W0110/Configs/BPC_C04-W0110_15V_IKrum1_96MHz_08-08-13"
// ;
// char AsciiconfigName[256] =
// "/home/lcd/CLIC_Testbeam_August2013/eudaq/TimepixProducer/Pixelman_SCL_2011_12_07/Default_Ascii_Config"
// ;
...
TimepixDevice::TimepixDevice() {
  ...
  mgrRegisterCallback("mpxctrl", MPXCTRL_CB_ACQPRESTART, &AcquisitionPreStarted,
                      0);
  ...
  // Get Info on device
  int info = mpxCtrlGetMedipixInfo(devId, &numberOfChips, &numberOfRows,
                                   chipBoardID, ifaceName);
  if (info == 0)
    cout << "[TimepixDevice] Number of chips : " << numberOfChips
         << " Number of rows : " << numberOfRows
         << " chipBoard ID : " << chipBoardID
         << " Interface Name : " << ifaceName << endl;

  else
    cout << "[TimepixDevice] board not found !! " << endl;
}

void TimepixDevice::Configure(const std::string &binary_config,
                              const std::string &ascii_config) {

  control = mpxCtrlLoadPixelsCfg(devId, binary_config.c_str(), true);
  cout << "[TimepixDevice] Load binary pixels config : " << binary_config
       << endl;
...
//		cout << "[TimepixProducer] THL Fine Readout = " <<
//dacVals_new[TPX_THLFINE] << endl;
//
...
int TimepixDevice::SetIkrum(int IKrum) {
  ...
  DACTYPE dacVals_new[14];
  control = mpxCtrlGetDACs(devId, dacVals_new, 14, chipnumber);
  cout << "[TimepixProducer] IKrum Readout = " << dacVals_new[TPX_IKRUM]
       << endl;
...
int TimepixDevice::SetTHL(int THL) {
  ...
  control = mpxCtrlGetDACs(devId, dacVals_new, 14, chipnumber);
  cout << "[TimepixProducer] Reading back THL Value , THL = "
       << dacVals_new[TPX_THLFINE] << endl;
...
int TimepixDevice::ReadDACs() {
  ...
  cout << "[TimepixProducer] IKrum Readout = " << dacVals_readout[TPX_IKRUM]
       << endl;
  cout << "[TimepixProducer] DISC Readout = " << dacVals_readout[TPX_DISC]
       << endl;
  cout << "[TimepixProducer] PREAMP Readout = " << dacVals_readout[TPX_PREAMP]
       << endl;
  cout << "[TimepixProducer] BUFFA Readout = " << dacVals_readout[TPX_BUFFA]
       << endl;
  cout << "[TimepixProducer] BUFFB Readout = " << dacVals_readout[TPX_BUFFB]
       << endl;
  cout << "[TimepixProducer] HIST Readout = " << dacVals_readout[TPX_HIST]
       << endl;
  cout << "[TimepixProducer] THLFINE Readout = " << dacVals_readout[TPX_THLFINE]
       << endl;
  cout << "[TimepixProducer] THLCOARSE Readout = "
       << dacVals_readout[TPX_THLCOARSE] << endl;
  cout << "[TimepixProducer] VCAS Readout = " << dacVals_readout[TPX_VCAS]
       << endl;
  cout << "[TimepixProducer] FBK Readout = " << dacVals_readout[TPX_FBK]
       << endl;
  cout << "[TimepixProducer] GND Readout = " << dacVals_readout[TPX_GND]
       << endl;
  cout << "[TimepixProducer] THS Readout = " << dacVals_readout[TPX_THS]
       << endl;
  cout << "[TimepixProducer] BIASLVDS Readout = "
       << dacVals_readout[TPX_BIASLVDS] << endl;
  cout << "[TimepixProducer] REFLVDS Readout = " << dacVals_readout[TPX_REFLVDS]
       << endl;
...
int TimepixDevice::PerformAcquisition(char *output) {
  ...
  // cout << "[timepixProducer] Starting acquisition to file " << output <<
  // endl;
  control = mpxCtrlPerformFrameAcq(devId, 1, acqTime, FSAVE_BINARY | FSAVE_U32,
                                   output);
