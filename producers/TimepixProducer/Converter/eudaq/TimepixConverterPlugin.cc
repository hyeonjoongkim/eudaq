...
public:
  mimtlu_exception(const std::string &message) : std::runtime_error(message){};
};
...
struct mimtlu_event {
  ...
  mimtlu_event(unsigned long int _timestamp, unsigned char _track,
               unsigned int _tlu) {
  ...
  mimtlu_event(const unsigned char *buf) { from_string(buf); }

  mimtlu_event(const std::vector<unsigned char> &buf) { from_vector(buf); }
  ...
  const unsigned char *to_char(void) { return txt; }
...
namespace eudaq {
  ...
  public:
    ...
    virtual void Initialize(const Event &bore, const Configuration &cnf) {
      ...
      //	TString str=
      //TString::Format("%s/TimepixProducer/FitParameters_2stepFit_v7-I10_W015.root",path);
  ...
  private:
    ...
    void ConvertLCIOHeader(lcio::LCRunHeader &header,
                           eudaq::Event const & /*bore*/,
                           eudaq::Configuration const & /*conf*/) const {
    ...
    bool ConvertLCIO(lcio::LCEvent &result, const Event &source) const {
      ...
      auto_ptr<lcio::LCCollectionVec> rawDataCollection(
          new lcio::LCCollectionVec(lcio::LCIO::TRACKERRAWDATA));
      auto_ptr<lcio::LCCollectionVec> zsDataCollection(
      ...
      for (size_t iPlane = 0; iPlane < numplanes; ++iPlane) {
        ...
        for (unsigned int i = 0; i < data.size() / 12; i++) {
          ...
          //	  double Energy;
          //	  if(A[pixx][pixy]!=0 and B[pixx][pixy] and C[pixx][pixy] and
          //D[pixx][pixy]){
          //	  	 Energy = (D[pixx][pixy]*A[pixx][pixy] + aWord -
          //B[pixx][pixy] +
          //sqrt((B[pixx][pixy]+D[pixx][pixy]*A[pixx][pixy]-aWord)*(B[pixx][pixy]+D[pixx][pixy]*A[pixx][pixy]-aWord)+4*A[pixx][pixy]*C[pixx][pixy]))/(2*A[pixx][pixy]);
          //		}
          //	  else {
          //	  	 Energy = (meanD*meanA + aWord - meanB +
          //sqrt((meanB+meanD*meanA-aWord)*(meanB+meanD*meanA-aWord)+4*meanA*meanC))/(2*meanA);
          //
          //	  }
          ZSDataTOT.push_back(aWord);

          // cout << "[DATA] " << ZSDataX[i] << " " << ZSDataY[i] << " " <<
          // ZSDataTOT[i] << endl;
        }
        ...
        for (unsigned i = 0; i < nPixel; i++) {
          ...
          // \n",nPixel,i, (signed short) plane.GetPixel(i, 0), (signed
          // short)plane.GetX(i) ,(signed short)plane.GetY(i));
      ...
      if (result.getEventNumber() == 0) {
        ...
        try {
          timepixSetupCollection = static_cast<LCCollectionVec *>(
              result.getCollection("timepixSetup"));
          timepixSetupExists = true;
        } catch (...) {
          timepixSetupCollection =
