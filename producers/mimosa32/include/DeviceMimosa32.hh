...
private:
  struct sockaddr_in
      fClientAddr; /* indirizzo del client che si collega al server*/
  int fNumChip;
  // Network
...
public:
  ...
  int create_client_tcp(int port, char *ip); // Creazione del socket client per
                                             // la comunicazione con il
                                             // dispositivo.
  int connect_client_tcp(
      int sd); // Connessione del socket client con il dispositivo.
  void close_client_tcp(int sd); // Chiusura del socket client.
  int read_file_cfg();           // Lettura del file di configurazione.
  int readout_event(int num_word_event, FILE *fp, char *fCDH, char *fDH,
                    char *fData); // Lettura dei dati dal dispositivo.
  void print_data(char *data, int pos_end,
                  FILE *fp); // Stampa dei dati su file e a video.
