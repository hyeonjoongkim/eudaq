...
// Restituisce -1 se fallisce la creazione del socket. Oppure il descrittore del
// socket.
//----------------------------------------------------------------------------------------------
...
int DeviceMimosa32::read_file_cfg() {
  ...
  while (file.good()) {
    ...
    while (word != NULL) {
      ...
      if (count == 0 && strcmp(word, char_ip) == 0) {
        isIp = 1;
      }
      if (count == 0 && strcmp(word, char_port) == 0) {
        isPort = 1;
      }
      if (count == 0 && strcmp(word, char_num_chip) == 0) {
        isNumChip = 1;
      }
      word = strtok(NULL, ": ");
      count++;
...
// Dalla word numero 2 alla word numero 9 abbiamo il CDH che viene salvato nella
// stringa fCDH.
// Dalla word numero 10 alla word numero 13 abbiamo il DH che viene salvato
// nella stringa fDH.
// Dalla word numero 14 in poi abbiamo i dati che vengono savati nella stringa
// fData.
// l'evento completto di CDH, DH e Dati viene salvato nella stringa fAllData.
// La stringa data_inv contienele stesse informazioni di fAllData e viene usata
// per la conversione
// dei dati in esadecimale che poi vengono scritti nel file.
// Restituisce 1 quando termina un evento. Viene passato il file dove scrivere i
// dati.
//----------------------------------------------------------------------------------------------
int DeviceMimosa32::readout_event(int num_word_event, FILE *fp, char *cdh,
                                  char *dh, char *all_data) {
  ...
  int i, n; // d e n contattori decrescenti usati rispettivamente per la stringa
            // istruzione e per la stringa dati.
  int num_byte_event = 0; // Numero totale di byte presi durante l'evento.
  int pos_start = 0; // indice di posizione di inizio per la memorizzazione
                     // della word nella struttura dati.
  int pos_end = 3; // indice di posizione di fine per la memorizzazione della
                   // word nella struttura dati.
...
// Viene passato il vettore di caratteri data che contiene i dati dell'evento
// comprensivi di CDH e DH.
...
// All'interno della funzione viene scritto il valore num_word_event tramite
// l'uso di un puntatore.
//----------------------------------------------------------------------------------------------
...
// Un carattere 0 o 1 deve essere convertito in bit di 0 o 1 per essere mandato
// al dispositivo.
// Una stringa istruzione che viene e' composta da 4 byte ovvero 32 bit ovvero32
// caratteri.
// Questi caratteri contenuti nella stringa buffer vengono convertiti in bit e
// inseriti nella stringa
...
// Se la conferma non arriva dopo due secondi va in timeout. Word contiena la
// word di istruzione da spedire.
//-------------------------------------------------------------------------------------------------
...
//-------------------------------------------------------------------------------------------------
int DeviceMimosa32::get_num_chip() { return fNumChip; }
// Restituisce il numero di chip
//-------------------------------------------------------------------------------------------------
int DeviceMimosa32::get_port() { return fPort; }
// Restituisce il numero ip
//-------------------------------------------------------------------------------------------------
void DeviceMimosa32::get_ip(char *ip) { strcpy(ip, fIp); }
