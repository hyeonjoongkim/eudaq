...
namespace eudaq {
  ...
  /** A helper class to read the byte sequence of big endian 32 bit data as 10
   * bit, 32 bit or
   *  40 bit data.
   *  This class only contains a reference to the data, not a copy, to avoid
   * copying of the data.
   */
  ...
  public:
    ...
    /** Helper function to get a 40 bit word with correct endinanness out of the
     * byte vector.
     *  The offset32bit is the position of the first 10 bit word within the
     * 32bit stream
     */
    unsigned short Get10bitWord(unsigned int index10bit,
                                unsigned int n40bitwords,
                                unsigned int offset32bit) const;

    /** Helper function to get a 32 bit word with correct endinanness out of the
     * byte vector.
     */
    ...
    /** Helper function to get a 40 bit word with correct endinanness out of the
     * byte vector.
     *  The offset32bit is the position of the first 40 bit word within the
     * 32bit stream
     */
  ...
  public:
    ...
    public:
      ...
      BadDataBlockException(const std::string &text) {
        message = "BadDataBlockException: " + text;
      }
  ...
  UCharBigEndianVec::UCharBigEndianVec(
      std::vector<unsigned char> const &datavec, bool altrowordsreversed)
  ...
  uint64_t UCharBigEndianVec::Get40bitWord(unsigned int index40bit,
                                           unsigned int n40bitwords,
                                           unsigned int offset32bit) const {
    // check position in data stream
    if (Get32bitWord(offset32bit - 8) != 0xFFFFFFFF)
      EUDAQ_WARN("AltroConverterPlugin::Get40bitWord: Suspicious position in "
                 "data stream, altro sequence does not start 8 words after rcu "
                 "header");

    // calculate the 8 bit offset in dependence whether the 40 bits words are in
    // reversed order or not
    unsigned int index8bit =
        offset32bit * 4 +
        (!_altrowordsreversed ? index40bit : n40bitwords - index40bit - 1) * 5;

    // The 32  bit words have big endian order. The 40 bit word is always
    // contained
    // in 2 consecutive 32 bit words,
    // the most significant bytes being in the second word (big endian 32 bit
    // sequence).
    // This means the 5 bytes are in a consecutive
    // sequence in the big endian data stream, but in reverse order. We just
    // have to pick them out
    // and place them in the int64_t word

    // Forunately this also works with the reversed 40 bit order, because 40
    // bits is always a
    // sequence of 5 bytes, whatever the other words are.

    uint64_t retval = static_cast<uint64_t>(_bytedata[index8bit]) |
  ...
  unsigned short
  UCharBigEndianVec::Get10bitWord(unsigned int index10bit,
                                  unsigned int n40bitwords,
                                  unsigned int offset32bit) const {
    // The way to get the bits out of the data stream is a sliding window
    // algorithm.
    // The sequence is always contained in two consecutive bytes. First the two
    // relevant bytes
    // are copied to a 16 bit word, then the 10 bit mask is placed at the
    // specific position.
    // The position depends on the 10 bit index (the second word is shifted by 2
    // bits, the third
    // by 4 bits and the fourth by 6 bits. The fifth word is shifted by 8 bits,
    // which means no
    ...
    {
      // Calculate the 8 bit index. Note that the first part is an integer
      // division, which
      // automatically generates the correct index ( 1*10/8=1 , 2*10/8=2,
      // 3*10/8=3, 4*10/8=5 etc)
    ...
    } else // reversed ordering of altro words, i. e. forward
    {
      ...
      unsigned int reversedindex10bit =
          (n40bitwords - (index10bit / 4) - 1) * 4 + (index10bit % 4);
      //      std::cout << "i10 "<< index10bit<< " ri10 "<< reversedindex10bit
      //      //		<< " n40 "<< n40bitwords<<std::endl;
    ...
    unsigned short data16bit =
        _bytedata[index8bit] | (_bytedata[index8bit + 1] << 8);

    // Calculate how far to shift the window, i. e. relative alignment of 10-bit
    // and 8-bit blocks
    unsigned short bitshift = (index10bit % 4) * 2;
    unsigned short mask = 0x3FF << bitshift; // 10-bit mask, shifted

    return (data16bit & mask) >>
           bitshift; // apply the mask and shift the result back to the LSB
  }

#if USE_LCIO
  bool
  AltroConverterPlugin::GetLCIOSubEvent(lcio::LCEvent &lcioevent,
                                        eudaq::Event const &eudaqevent) const {
    // try to cast the eudaq event to RawDataEvent
    eudaq::RawDataEvent const &rawdataevent =
        dynamic_cast<eudaq::RawDataEvent const &>(eudaqevent);
    ...
    int formatversion =
        atoi(eudaqevent.GetTag("Data format version", "0").c_str());

    // note: atoi will return 0 if it fails, so formatversion is 0 if the
    // version was not
    ...
    lcio::LCCollectionVec *altrocollection =
        new lcio::LCCollectionVec(lcio::LCIO::TRACKERRAWDATA);
    ...
    try {
      ...
      for (size_t block = 0; block < rawdataevent.NumBlocks(); block++) {
        ...
        if (eventlength + 1 != altrodatavec.Size()) {
          std::cout << "event length = " << (eventlength + 1) * 4
                    << " data size " << altrodatavec.Size() << std::endl;
          EUDAQ_WARN("AltroConverterPlugin::GetLCIOEvent: Wrong event length "
                     "given in raw data");
        }
        ...
        //	if ( (headerlength == 4) )
        //	{
        //	    EUDAQ_WARN("AltroConverterPlugin::GetLCIOEvent: Suspicious
        //header length given in raw data");
        ...
        if (blockID != 0x22222222) {
          EUDAQ_THROW(
              "AltroConverterPlugin::GetLCIOEvent: Error reading event header");
        }

        // read the TLU event number (7th word)
        unsigned int tlu_eventnumber = 0;
        // the 7th bit in the header only exists from data format version 4.1
        // on,
        ...
        // the two extra words are the event length and the header length, which
        // are not counted
        // in the length word
        ...
        while (rcublockstart < altrodatavec.Size()) {
          ...
          unsigned int rcublocklength =
              altrodatavec.Get32bitWord(rcublockstart);
          unsigned int rcuID = altrodatavec.Get32bitWord(rcublockstart + 1);
          //	    std::cout << "DEBUG: rcublocklength = "<<rcublocklength<< "
          //at rcublockstart"
          //		      << rcublockstart << std::endl;

          // check that we have the correct position of the rcu block, third
          // word has to be 0xFFFFFFFF
          if (altrodatavec.Get32bitWord(rcublockstart + 2) != 0xFFFFFFFF) {
            std::cerr << "invalid rcu identifier 0x" << std::hex
                      << altrodatavec.Get32bitWord(rcublockstart + 2)
                      << std::dec << std::endl;
          ...
          unsigned int rcutrailerlength =
              altrodatavec.Get32bitWord(rcublockstart + rcublocklength) & 0x7F;
          //	    std::cout << "DEBUG: rcutrailerlength "<<rcutrailerlength <<
          //std::endl;

          // the number of 40 bit words is the first entry in the rcu trailer
          unsigned int n40bitwords = altrodatavec.Get32bitWord(
              rcublockstart + rcublocklength - rcutrailerlength + 1);

          //	    std::cout << "DEBUG: rcu block contains "<<n40bitwords<<"
          //40bit words"<< std::endl;

          // read the sequence of 40 bit altro words backward. It is made up of
          // blocks
          // ending with a trailer word

          // the position of the next trailer word to read
          // use a signed int, it will become negative if there are no more
          // words to read
          int altrotrailerposition = n40bitwords - 1;
          //	    std::cout << "DEBUG: altrotrailerposition" <<
          //altrotrailerposition << std::endl;
          ...
          //			  << std::setw(10)
          //			  << altrodatavec.Get40bitWord(i, n40bitwords,
          //rcublockstart + 10 ) << "\t"
          //			  << std::setw(3)
          //			  << altrodatavec.Get10bitWord(4*i+3, n40bitwords,
          //rcublockstart + 10 )<<" "
          //			  << std::setw(3)
          //			  << altrodatavec.Get10bitWord(4*i+2, n40bitwords,
          //rcublockstart + 10 )<<" "
          //			  << std::setw(3)
          //			  << altrodatavec.Get10bitWord(4*i+1, n40bitwords,
          //rcublockstart + 10 )<<" "
          //			  << std::setw(3)
          //			  << altrodatavec.Get10bitWord(4*i  , n40bitwords,
          //rcublockstart + 10 )
          //			  <<std::dec<<std:: endl;
          //	    }
          ...
          //	    {
          //		std::cout << std::hex << std::setfill('0')
          //			  << std::setw(10)<< altrodatavec.Get40bitWord(i,
          //rcublockstart + 10 )
          //			  <<std::dec<<std:: endl;
          //	    }
          ...
          //	    for (unsigned int i= rcublockstart ; i <= rcublockstart +
          //rcublocklength ; i++)
          //	    {
          //		std::cout << std::hex << std::setfill('0')
          ...
          while (altrotrailerposition > 0) {
            ...
            if (altrotrailerposition == previous_altrotrailer) {
              throw BadDataBlockException(
                  "Endless loop, reading same altro trailer position twice.");
            } else {
              previous_altrotrailer = altrotrailerposition;
            }

            uint64_t altrotrailer = altrodatavec.Get40bitWord(
                altrotrailerposition, n40bitwords, rcublockstart + 10);
            ...
            if ((altrotrailer & 0xFFFC00F000ULL) != 0xAAA800A000ULL) {
              ...
              //    }
              //    else // now something really went wrong, and we cannot
              //    recover
              //    {
              //	  std::cerr << "error wrong altro trailer word 0x"<<
              //std::hex << altrotrailer << std::dec << std::endl;
              //	  EUDAQ_THROW("Invalid Altro trailer word");
              //    }
            }

            // even if the channel is broken we have to continue and read the
            // number of 10bit words contine the event
            unsigned int n10bitwords = (altrotrailer & 0x3FF0000) >> 16;
            unsigned int channelnumber = altrotrailer & 0xFFF;

            //		std::cout <<"DEBUG: altro block on channel "<< channelnumber
            //<<" contains "
            ...
            int nfillwords =
                (4 - n10bitwords % 4) %
                4; // the number of fill words to complete the 40 bit words
            int index10bit = (altrotrailerposition * 4) - nfillwords - 1;

            // if (channel_is_broken)
            //  {
            //    std::cout << "skipping broken altro block on channel " <<
            //    channelnumber << " in event " << lcioevent.getEventNumber() <<
            //    std::endl;
            //  }
            // else // alto block is ok process it
            {
              ...
              while (index10bit >
                     (altrotrailerposition * 4) -
                         static_cast<int>(n10bitwords + nfillwords)) {
                if ((altrotrailerposition * 4) -
                        static_cast<int>(n10bitwords + nfillwords) <
                    0) {
                  throw BadDataBlockException(
                      "Trying to read backward to negative 10bit index.");
                }

                // check for endless loop
                if (index10bit == previous_index10bit) {
                  std::cout << "trying to read 10bit index " << index10bit
                            << " twice" << std::endl;
                  std::cout << std::hex << "altrotrailer " << altrotrailer
                            << std::dec << std::endl;
                  throw BadDataBlockException(
                      "Endless loop, reading same 10bit index twice.");
                } else {
                  previous_index10bit = index10bit;
                }

                // length of this data record, it also counts this length word
                // and the timestamp word, so the number of data samples is
                // length - 2
                unsigned short length = altrodatavec.Get10bitWord(
                    index10bit, n40bitwords, rcublockstart + 10);
                ...
                unsigned short timestamp = altrodatavec.Get10bitWord(
                    index10bit - 1, n40bitwords, rcublockstart + 10);

                //		    std::cout <<"DEBUG: found pulse with "<<
                //ndatasamples
                //			      <<" ndatasamples at time index " <<
                //timestamp
                //			      <<" on channel " << channelnumber <<
                //std::endl;

                // the time stamp in the data stream corresponds to the last
                // sample
                // in lcio it has to be the first sample
                timestamp -= (ndatasamples - 1);

                // create the lcio data record
                lcio::TrackerRawDataImpl *altrolciodata =
                    new lcio::TrackerRawDataImpl;

                altrolciodata->setCellID0(channelnumber);
                //		    std::cout << "rcu ID is " << rcuID<<
                //std::endl;
                ...
                // fill the data samples into a vactor and add it to the lcio
                // raw data
                ...
                for (unsigned int sample = index10bit - length + 1, i = 0;
                     ...
                     sample++, i++) {
                  datasamples[i] = altrodatavec.Get10bitWord(
                      sample, n40bitwords, rcublockstart + 10);
                }
            ...
            // It is n10bitwords/4, rounded up (so we have to perform a floating
            // point division)
      ...
      std::cout << "Event " << lcioevent.getEventNumber()
                << " contains bad data block, skipping it " << e.what()
                << std::endl;
    ...
    // If the collection is not empty, add the collection to the event and
    // return the event
    lcioevent.addCollection(altrocollection, "AltroRawData");
    return true;
  }
#else  // if USE_LCIO
  bool AltroConverterPlugin::GetLCIOSubEvent(lcio::LCEvent &,
                                             eudaq::Event const &) const {
    return false;
  }
#endif // IF USE_LCIO

  bool AltroConverterPlugin::GetStandardSubEvent(StandardEvent &,
                                                 eudaq::Event const &) const {
