...
public:
   ...
   * byte vector.
   *  The offset32bit is the position of the first 10 bit word within the 32bit
   * stream
   */
  ...
  /** Helper function to get a 40 bit word with correct endinanness out of the
   * byte vector.
   *  The offset32bit is the position of the first 40 bit word within the 32bit
   * stream
   */
...
uint64_t UCharBigEndianVec::Get40bitWord(unsigned int index40bit,
  ...
  // sequence in the big endian data stream, but in reverse order. We just have
  // to pick them out
  // and place them in the int64_t word

  // Forunately this also works with the reversed 40 bit order, because 40 bits
  // is always a
...
unsigned short UCharBigEndianVec::Get10bitWord(unsigned int index10bit,
  ...
  // relevant bytes
  // are copied to a 16 bit word, then the 10 bit mask is placed at the specific
  // position.
...
#if USE_LCIO
bool AltroConverterPlugin::GetLCIOSubEvent(
    lcio::LCEvent &lcioevent, eudaq::Event const &eudaqevent) const {
  ...
  // note: atoi will return 0 if it fails, so formatversion is 0 if the version
  // was not
  ...
  try {
    ...
    for (size_t block = 0; block < rawdataevent.NumBlocks(); block++) {
      ...
      if (eventlength + 1 != altrodatavec.Size()) {
        std::cout << "event length = " << (eventlength + 1) * 4 << " data size "
                  << altrodatavec.Size() << std::endl;
      ...
      while (rcublockstart < altrodatavec.Size()) {
        ...
        // check that we have the correct position of the rcu block, third word
        // has to be 0xFFFFFFFF
        if (altrodatavec.Get32bitWord(rcublockstart + 2) != 0xFFFFFFFF) {
          std::cerr << "invalid rcu identifier 0x" << std::hex
                    << altrodatavec.Get32bitWord(rcublockstart + 2) << std::dec
                    << std::endl;
        ...
        // use a signed int, it will become negative if there are no more words
        // to read
        ...
        while (altrotrailerposition > 0) {
          ...
          if ((altrotrailer & 0xFFFC00F000ULL) != 0xAAA800A000ULL) {
            ...
            //    {
            //	  std::cerr << "error wrong altro trailer word 0x"<< std::hex <<
            //altrotrailer << std::dec << std::endl;
          ...
          // else // alto block is ok process it
          {
            ...
            while (index10bit >
              ...
              // and the timestamp word, so the number of data samples is length
              // - 2
              ...
              // fill the data samples into a vactor and add it to the lcio raw
              // data
  ...
  // If the collection is not empty, add the collection to the event and return
  // the event
