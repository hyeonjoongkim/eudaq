...
void Status::Execute(AltroProducer *, RUNSTATUS *rs) {
  ...
  std::stringstream s;
  s << "STATUS DAQ " << rs->daq << " RUN " << rs->run << " LOG " << rs->logging
    << " MON " << rs->monevents << " EVT " << rs->nbevents << " TYPE "
    << rs->type << " MODE " << rs->runmode << " RUNNB " << rs->runnb << " ERR "
    << rs->feerrors;
  EUDAQ_INFO(s.str());
}
...
StartRun::StartRun(unsigned int monevents, int logging, int type, int maxevents,
                   int maxmonevents)
...
void PCA::Execute(AltroProducer *producer, RUNSTATUS *rs) {
  ...
  if (rs->daq == 0) {
    ...
    pca.dac = _dac;
    // calculate all other pararam of the pca from the given shiftregister and
    // dac
