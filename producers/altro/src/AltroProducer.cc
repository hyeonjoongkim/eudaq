...
namespace eudaq {
  ...
  namespace altroproducer {
    ...
    void Status::Execute(AltroProducer *, RUNSTATUS *rs) {
      ...
      std::stringstream s;
      s << "STATUS DAQ " << rs->daq << " RUN " << rs->run << " LOG "
        << rs->logging << " MON " << rs->monevents << " EVT " << rs->nbevents
        << " TYPE " << rs->type << " MODE " << rs->runmode << " RUNNB "
        << rs->runnb << " ERR " << rs->feerrors;
      EUDAQ_INFO(s.str());
    }
    ...
    void Power::Execute(AltroProducer *producer, RUNSTATUS *rs) {
      ...
      for (unsigned int i = 0; i <= RCU_MAX_ID; i++) {
        std::stringstream s;
        s << "Power Status S" << i << " " << fecpower[i].status << " RCU" << i
          << " " << fecpower[i].power << " P" << fecpower[i].poweredon;
        EUDAQ_INFO(s.str());
      }
    }
    ...
    void PCA::Execute(AltroProducer *producer, RUNSTATUS *rs) {
      int retstat = 0;
      /* decode what to do - but if daq is active we can only return the current
       * status */
      if (rs->daq == 0) {
        PCA16 pca;
        pca.shiftreg = _shiftRegister;
        pca.dac = _dac;
        // calculate all other pararam of the pca from the given shiftregister
        // and dac
      ...
      std::stringstream s;
      s << "PCA Status ShiftRegister " << pcasettings->shiftreg << " DAC "
        << pcasettings->dac << " Error " << retstat;
      EUDAQ_INFO(s.str());
    ...
    void AltroProducer::Event(volatile uint32_t *altrodata, int length) {
      eudaq::RawDataEvent ev("TimepixEvent", GetRunNumber(),
                             GetIncreaseEventNumber());

      // a data block of unsigned char, in this the data is stored in little
      // endian
      unsigned char *serialdatablock =
          new unsigned char[length * sizeof(uint32_t)];
      ...
      for (int i = 0; i < length; i++) {
        for (unsigned int j = 0; j < sizeof(uint32_t); j++) {
          ...
          serialdatablock[sizeof(uint32_t) * i + j] =
              (altrodata[i] & (0xFF << 8 * j)) >>
              (sizeof(uint32_t) - j - 1) * 8;
        }
      }
    ...
    void AltroProducer::OnConfigure(const eudaq::Configuration &param) {
      ...
      if (GetRunActive()) {
        ...
        SetStatus(eudaq::Status::LVL_WARN,
                  "Cannot reconfigure, run is active!");
        return;
      }
    ...
    void AltroProducer::OnPrepareRun(unsigned /*param*/) {
      ...
      if (GetRunActive()) {
        // do the warning stuff
        SetStatus(eudaq::Status::LVL_WARN,
                  "Cannot prepare run, run is active!");
        return;
      }
      // send start daq command
      // FIXME: where to get the parameters from? Config file of the run
      // control?
    ...
    void AltroProducer::OnStartRun(unsigned param) {
      ...
      SendEvent(eudaq::RawDataEvent::BORE(
          "AltroEvent", param)); // send param instead of GetRunNumber
      ...
      // FIXME: Where to get the parameters from? Config file of the run
      // control?
      // CommandPush( new StartRun(monevents, logging, type, maxevents,
      // maxmonevents ) );
    }

    void AltroProducer::OnStopRun() {
      //    SendEvent(eudaq::RawDataEvent::EORE("TimepixEvent", GetRunNumber(),
      //    GetEventNumber()));
    ...
    void AltroProducer::OnUnrecognised(const std::string &cmd,
