...
namespace eudaq {
  ...
  namespace altroproducer {
    ...
    class Command {
    public:
      //    /** The available commands that the command receiver can pass to the
      //    readout loop
      //     *  (runhandler).
      //     *  All commands of the ilcserver--readout communication must be
      //     implemneted.
      //     *  \li \c *CLI: No needed, producer is a class instance in the same
      //     executable
      ...
      /** Execute the command. This is the purely virtual function which has to
       *be implemented
       *
       */
    ...
    public:
      ...
      Power(PowerCommands
                command); ///< command can be status (0), on (1) or off (2)
      virtual ~Power() {}
    ...
    public:
      StartRun(unsigned int monevents, int logging, int type, int maxevents = 0,
               int maxmonevents = 0);
      virtual ~StartRun() {}
    ...
    public:
       ...
       *  the conversion, which is faster (memory does not have to be allocated
       * and
       *  released).
       */
       ...
       *  mutex. The non-increased version is returned (like in the
    ...
    public:
      /** As the command receiver runs in a separate thread we need a queue to
       * ensure all
       *  commands are executed. Do not access directly, only use the thread
       * safe CommandPush() and
       *  CommandPop()!
       */
      ...
      /** The thread safe way to pop a command fom the command queue. If the
       * queue is emty
       *  it returns 0. Attention: You have to delete the command after
       * executing it!
       */
      Command *CommandPop();

      /** The thread safe way to get a reference to the first command fom the
       * command queue
       *  without removing it from the queue
       */
      Command *CommandFront();

      // all data members have to be protected by mutex since they can be
      // accessed by multiple
      // threads
      /// status wether a data run is active
      bool m_runactive;
      pthread_mutex_t m_runactive_mutex;
      /// The run number
      unsigned m_run;
      pthread_mutex_t m_run_mutex;
      /// The event number
      unsigned m_ev;
      pthread_mutex_t m_ev_mutex;
    };
