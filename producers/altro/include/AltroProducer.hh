...
public:
  ...
  /** Execute the command. This is the purely virtual function which has to be
   *implemented
   *
   */
...
public:
  ...
  Power(
      PowerCommands command); ///< command can be status (0), on (1) or off (2)
  virtual ~Power() {}
...
public:
   ...
   *  mutex. The non-increased version is returned (like in the post-increment
   * operator).
   */
...
public:
   ...
   * ensure all
   *  commands are executed. Do not access directly, only use the thread safe
   * CommandPush() and
   *  CommandPop()!
   */
  ...
  /** The thread safe way to pop a command fom the command queue. If the queue
   * is emty
   *  it returns 0. Attention: You have to delete the command after executing
   * it!
   */
  Command *CommandPop();
  ...
  // all data members have to be protected by mutex since they can be accessed
  // by multiple
  // threads
