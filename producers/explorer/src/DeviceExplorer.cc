...
// Costructor
//----------------------------------------------------------------------------------------------
DeviceExplorer::DeviceExplorer() : data(), sc(), cmdRdy(system(NULL)) {
  data[0] = -1;
  data[1] = -1;
  sc[0] = -1;
  sc[1] = -1;
...
//----------------------------------------------------------------------------------------------
int DeviceExplorer::create_server_udp(
    int fecn, int port /* = PORT */) { // create udp serves for fecs
  int server;
  char ip[14];
  sprintf(ip, "10.0.%d.3", fecn); // build the IPs
  if ((fecn != 0 && fecn != 1) || (port != 6006 && fecn == 1) ||
      (port != 6007 && port != 6006 && fecn != 0)) {
...
void DeviceExplorer::close_server_udp(int sd) { close(sd); }
...
bool DeviceExplorer::Configure() {
  if (sc[0] >= 0)
    StopDAQ();
  if (cmdRdy) {
    system("../producers/explorer/srs-software/ConfigureExplorer.sh");
    return true;
  } else
    return false;
}
...
bool DeviceExplorer::ConfigureDAQ() {
  if (cmdRdy) {
    system("../producers/explorer/srs-software/ConfigureDAQ.sh");
  } else
    return false;
  cmdRdy = false;
...
bool DeviceExplorer::GetSingleEvent() {
  if (sc[0] >= 0) {
    const unsigned char re[20] = {
        ...
        0xFF, 0x00, 0x00, 0x00, 0x16, 0x00, 0x00, 0x00, 0x06}; // rising edge
                                                               // for the event
                                                               // request
    const unsigned char fe[20] = {
        0x80, 0x00, 0x00, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xAA, 0xBB, 0xFF,
        0xFF, 0x00, 0x00, 0x00, 0x16, 0x00, 0x00, 0x00, 0x04}; // falling edge
                                                               // for the event
                                                               // request
