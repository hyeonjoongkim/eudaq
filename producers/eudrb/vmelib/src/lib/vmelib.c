 ...
 *Version        :       1.3
 *--------------------------------------------------------------------------------------
 *License        :       You are free to use this source files for your own
 *development as long
 *                               as it stays in a public research context. You
 *are not allowed to use it
 *                               for commercial purpose. You must put this
 *header with
 *                               authors names in all development based on this
 *library.
 *--------------------------------------------------------------------------------------
 */
...
static int dodmaxfer(int bytecount, unsigned long int srcaddress,
                     unsigned long int dstaddress, int xfer);
void set_parameters(int fdOut, int window_number, unsigned long int address,
                    int xferRate2esst, int addrSpace, int maxDataWidth,
                    int xferProtocol, int userAccessType, int dataAccessType);
 ...
 *       sono stati impostati correttamente dopo una  chiamata ioctl
 * VME_IOCTL_SET_OUTBOUND
 */
static void outbound_ctl(int fdOut, vmeOutWindowCfg_t *vmeOutSet) {
 ...
 *
 *       Secondo me serve a trasferire il contenuto della cache perche' alloco
 *il vettore di
 *       destinazione poi lo porto in ram effettuo il trasferimento
 *
 *       il TSI148 prende il controllo della RAM ed effettua un trasferimento ad
 *alta banda
 *       poi quando il processore riprende il controllo della ram bisogna
 *svuotare prima la cache del
...
static int dodmaxfer(int bytecount, unsigned long int srcaddress,
  ...
  vmeDma.srcVmeAttr.xferProtocol =
      xfer; /*Protocollo di trasferimento
  ...
  vmeDma.dstAddr =
      dstaddress; /*Indirizzo destinazione o Buffer di destinazione*/
  ...
  vmeDma.dstVmeAttr.xferProtocol =
      xfer; /*Protocollo di trasferimento
             *MBLT,2eVME,2eSST hanno 64 bit per i dati
             *ma non bisogna cambiare maxDataWidth=VME_D32
             */
...
void set_parameters(int fdOut, int window_number, unsigned long int address,
  ...
  if (vme_prevspace != addrSpace || vme_prevaddr != (address & MASK_HIGH)) {
     ...
     */
    vmeOutSet.windowNbr =
        window_number;          /* "Numero della Finestra"
    ...
    vmeOutSet.windowSizeL =
        WINDOW_SIZE; /* Grandezza della finestra impostato a 16M*/
    ...
    vmeOutSet.xferRate2esst =
        xferRate2esst; /* Abilitazione del clock
    ...
    vmeOutSet.xferProtocol = xferProtocol; /*
                                            ...
                                            */
    vmeOutSet.userAccessType =
        userAccessType; /* Accesso di tipo User/Supervisor OTAT*/
    vmeOutSet.dataAccessType =
        dataAccessType; /* Accesso di tipo Data/Program OTAT*/
    ...
    outbound_ctl(fdOut,
                 &vmeOutSet); /*Controllo se i parametri settati sono stati
...
int vme_A32_D32_User_Data_SCT_read(int fdOut, unsigned long int *readdata,
                                   unsigned long int address) {
  size_t n =
      0; /*Per controllare se l'operazione di read e' andata a buon fine*/
  size_t nbyte = 0; /*Byte da leggere per la read*/
  off_t offset;     /*Offset della lseek*/
  set_parameters(fdOut, 0, address, VME_SSTNONE, VME_A32, VME_D32, VME_SCT,
                 VME_USER, VME_DATA);
  /*
  ...
  offset =
      lseek(fdOut, address & MASK_LOW,
            SEEK_SET); /*Impostazione dell'offset dal base address dello slave*/
...
int vme_A32_D16_User_Data_SCT_read(int fdOut, unsigned short int *readdata,
                                   unsigned long int address) {
  size_t n =
      0; /*Per controllare se l'operazione di read e' andata a buon fine*/
  size_t nbyte = 0; /*Byte da leggere per la read*/
  ...
  set_parameters(fdOut, 0, address, VME_SSTNONE, VME_A32, VME_D16, VME_SCT,
                 VME_USER, VME_DATA);
  /*
   *       Operazione di lettura dal bus VME
   *       1-Impostazione della parte meno significativa
...
int vme_A24_D16_User_Data_SCT_read(int fdOut, unsigned short int *readdata,
                                   unsigned long int address) {
  size_t n =
      0; /*Per controllare se l'operazione di read e' andata a buon fine*/
  size_t nbyte = 0; /*Byte da leggere per la read*/
  off_t offset;     /*Offset della lseek*/
  set_parameters(fdOut, 0, address, VME_SSTNONE, VME_A24, VME_D16, VME_SCT,
                 VME_USER, VME_DATA);
  /*
   *       Operazione di lettura dal bus VME
   *       1-Impostazione della parte meno significativa
...
int vme_A24_D32_User_Data_SCT_read(int fdOut, unsigned long int *readdata,
                                   unsigned long int address) {
  size_t n =
      0; /*Per controllare se l'operazione di read e' andata a buon fine*/
  size_t nbyte = 0; /*Byte da leggere per la read*/
  off_t offset;     /*Offset della lseek*/
  set_parameters(fdOut, 0, address, VME_SSTNONE, VME_A24, VME_D32, VME_SCT,
                 VME_USER, VME_DATA);
  /*
   *       Operazione di lettura dal bus VME
   *       1-Impostazione della parte meno significativa
...
int vme_A32_D32_User_Data_SCT_write(int fdOut, unsigned long int writedata,
                                    unsigned long int address) {
  size_t n =
      0; /*Per controllare se l'operazione di write e' andata a buon fine*/
  size_t nbyte = 0; /*Byte da leggere per la read*/
  off_t offset;     /*Offset della lseek*/
  set_parameters(fdOut, 0, address, VME_SSTNONE, VME_A32, VME_D32, VME_SCT,
                 VME_USER, VME_DATA);
...
int vme_A32_D16_User_Data_SCT_write(int fdOut, unsigned short int writedata,
                                    unsigned long int address) {
  size_t n =
      0; /*Per controllare se l'operazione di write e' andata a buon fine*/
  size_t nbyte = 0; /*Byte da leggere per la read*/
...
int vme_A24_D32_User_Data_SCT_write(int fdOut, unsigned long int writedata,
                                    unsigned long int address) {
  size_t n =
      0; /*Per controllare se l'operazione di write e' andata a buon fine*/
  size_t nbyte = 0; /*Byte da leggere per la read*/
...
int vme_A24_D16_User_Data_SCT_write(int fdOut, unsigned short int writedata,
                                    unsigned long int address) {
  size_t n =
      0; /*Per controllare se l'operazione di write e' andata a buon fine*/
  size_t nbyte = 0; /*Byte da leggere per la read*/
...
int vme_A32_D32_User_Data_MBLT_read(int bytecount, unsigned long int srcaddress,
                                    unsigned long int *dmadstbuffer) {
  int i = 0;
...
int vme_A32_D64_User_Data_MBLT_read(int bytecount, unsigned long int srcaddress,
                                    unsigned long int *dmadstbuffer) {
  int i = 0;
  /*Inizializzazione della memoria*/
