...
namespace {
  ...
  static void CheckDriver() {
    ...
    if (fd < 0) {
      EUDAQ_THROW("Unable to open VME control (errno=" + to_string(errno) +
                  "). " + msg);
    }
    ...
    if (status < 0) {
      close(fd);
      EUDAQ_THROW("Unable get VME Info (errno=" + to_string(errno) + "). " +
                  msg);
    }
    ...
    if (status < 0) {
      close(fd);
      EUDAQ_THROW("Unable set VME requestor (errno=" + to_string(errno) +
                  "). " + msg);
    }
    ...
    close(fd);
    if (status < 0) {
      EUDAQ_THROW("Unable get VME requestor (errno=" + to_string(errno) +
                  "). " + msg);
    }

#define BAD(a) (VmeReq.a != VmeReq2.a)
    if (BAD(requestLevel) || BAD(fairMode) || BAD(releaseMode) ||
        BAD(timeonTimeoutTimer) || BAD(timeoffTimeoutTimer)) {
...
#undef BAD
  ...
  static addressMode_t ToAddrSpace(int width) {
    return width == VMEInterface::A16
               ? VME_A16
               : width == VMEInterface::A24
                     ? VME_A24
                     : width == VMEInterface::A32
                           ? VME_A32
                           : width == VMEInterface::A64 ? VME_A64 : VME_CRCSR;
  }
  static int ToProto(int proto) {
    return proto == VMEInterface::PSCT
               ? VME_SCT
               : proto == VMEInterface::PBLT
                     ? VME_BLT
                     : proto == VMEInterface::PMBLT
                           ? VME_MBLT
                           : proto == VMEInterface::P2eVME
                                 ? VME_2eVME
                                 : proto == VMEInterface::P2eSST
                                       ? VME_2eSST
                                       : proto == VMEInterface::P2eSSTB
                                             ? VME_2eSSTB
                                             : 0;
  }
  ...
  static inline void asm_dcbf(unsigned char *) { __asm__("dcbf 0,3"); }
  static inline void asm_sync() { __asm__("sync"); }
#else
...
TSI148SingleInterface::TSI148SingleInterface(unsigned long base,
                                             unsigned long size, int awidth,
                                             int dwidth, int proto, int sstrate)
    : VMEInterface(base, size, awidth, dwidth, proto, sstrate), m_chan(-1),
      m_fd(-1) {
  CheckDriver();
  OpenDevice();
  if (m_fd == -1) {
...
void TSI148SingleInterface::OpenDevice() {
  ...
  for (m_chan = MAX_CHANNEL; m_chan >= 0; --m_chan) {
    ...
    // std::cout << "DEBUG: VME trying channel " << m_chan << ", fd = " << m_fd
    // << std::endl;
    if (m_fd != -1)
      break;
  }
...
void TSI148SingleInterface::DoRead(unsigned long offset, unsigned char *data,
                                   size_t size) {
  ...
  if (n != size) {
    EUDAQ_THROW("Error: VME read failed at " + to_string(m_base) + "+" +
                to_string(offset) + "=" +
                to_string(eudaq::hexdec(m_base + offset, 8)) + " (code = " +
                to_string(errno) + ")");
  }
}

void TSI148SingleInterface::DoWrite(unsigned long offset,
  ...
  if (n != size) {
    EUDAQ_THROW("Error: VME write failed at offset " + to_string(offset) +
                ", wrote " + to_string(n) + " of " + to_string(size) +
                " bytes"
                " (code = " +
                to_string(errno) + ")");
  }
...
void TSI148SingleInterface::SysReset() { DoSysReset(); }
...
TSI148DMAInterface::TSI148DMAInterface(unsigned long base, unsigned long size,
                                       int awidth, int dwidth, int proto,
                                       int sstrate)
...
#define VMESET(var, val)                                                       \
  m_params->srcVmeAttr.var = val;                                              \
...
void TSI148DMAInterface::DoRead(unsigned long offset, unsigned char *data,
                                size_t size) {
  int status = 0;
  ...
  asm_sync();
  // std::cout << "DEBUG: dst address = " << eudaq::hexdec((unsigned long)data)
  // << std::endl;
  // #define DBG(x) std::cout << "VME DEBUG: " #x " = " << m_params->x <<
  // std::endl
  //   DBG(maxPciBlockSize);
  //   DBG(maxVmeBlockSize);
  //   DBG(byteCount);
...
void TSI148DMAInterface::DoWrite(unsigned long offset,
                                 const unsigned char *data, size_t size) {
...
void TSI148DMAInterface::SysReset() { DoSysReset(); }
