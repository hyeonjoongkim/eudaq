...
static FILE *tracefile() {
  static FILE *fp = 0;
  if (!fp)
    fp = fopen("vmetrace.txt", "w");
  return fp;
}
static void vmetrace(char *mode, int aw, int dw, unsigned long addr,
                     unsigned long data) {
...
//     : m_size(0), m_capacity(capacity), m_buf((T*)malloc(capacity * sizeof
//     (T)))
//     {
//       std::cout << "DEBUG: buffer = " << eudaq::hexdec((unsigned long)m_buf)
//       << std::endl;
//     }

//   void clear() { m_size = 0; }
//   void resize(size_t size) { m_size = size; /* TODO: throw error if size >
//   capacity */ }
...
//   const unsigned char * Buffer() const { return
//   eudaq::constuchar_cast(m_buf); }
...
public:
  ...
  VMEInterface(unsigned long base, unsigned long size, int awidth = A32,
               int dwidth = D32, int proto = PSCT, int sstrate = SSTNONE);

  virtual void SetWindow(unsigned long base,
                         unsigned long size = 0 /*0=unchanged*/);
  virtual void SetWindow(unsigned long base, unsigned long size, int awidth,
                         int dwidth, int proto = PSCT, int sstrate = SSTNONE);
  ...
  unsigned long Read(unsigned long offset) { return Read(offset, 0UL); }
  //   unsigned long Read(unsigned long offset) {
  //     unsigned long data = 0;
  //     return Read(offset, data);
  //   }
  template <typename T>
  std::vector<T> &Read(unsigned long offset, std::vector<T> &data) {
...
protected:
  virtual void DoRead(unsigned long offset, unsigned char *data,
                      size_t size) = 0;
  virtual void DoWrite(unsigned long offset, const unsigned char *data,
                       size_t size) = 0;
...
class VMEFactory {
public:
  static VMEptr Create(unsigned long base, unsigned long size, int awidth,
                       int dwidth, int proto,
                       int sstrate = VMEInterface::SSTNONE);
  static VMEptr Create(unsigned long base, unsigned long size) {
    return Create(base, size, VMEInterface::A32, VMEInterface::D32,
                  VMEInterface::PSCT);
  }
