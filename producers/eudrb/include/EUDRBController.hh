...
public:
  ...
  std::string PostConfigure(const eudaq::Configuration &param,
                            int master); // Download pedestals, send M26 start
                                         // pulse, returns ped file name if any
  ...
  bool
  EventDataReady(double timeout = 1.0); /// Returns false on timeout, else true
  int EventDataReady_size(
      double timeout = 1.0); /// Returns 0 on timeout, else event size
...
private:
  ...
  static pedestal_t ReadPedestals(const std::string &filename, float sigma,
                                  int det);
