...
namespace eudaq {
  ...
  namespace {
    ...
    static const char *DET_NAMES[] = {"NONE",    "MIMOSA5",  "MIMOSTAR2",
                                      "MIMOTEL", "MIMOSA18", "MIMOSA26"};
    static const int NUM_DETS = sizeof DET_NAMES / sizeof *DET_NAMES;

    template <typename T>
    inline T getpar(const eudaq::Configuration &param, int id,
                    const std::string &name, const T &def) {
      return param.Get("Board" + to_string(id) + "." + name, name, def);
    }
    inline std::string getpar(const eudaq::Configuration &param, int id,
                              const std::string &name, const char *def) {
      return param.Get("Board" + to_string(id) + "." + name, name,
                       std::string(def));
    }

    static int decode_offset_mimotel(int x, int y) {
      if (x >= 256 || y >= 256)
        EUDAQ_THROW("Bad coordinates in pedestal file");
      unsigned subm = x >> 6;
      if (subm == 1 || subm == 2)
        subm = 3 - subm; // fix for submatrix ordering
      x %= 64;
      return (x + 2) | (y << 9) |
             (subm << 18); // x+2, because 0 and 1 are dummy pixels
    }
    ...
    static int decode_offset_mimosa18(int x, int y) {
      ...
      if (subm == 1 || subm == 3)
        subm = 4 - subm; // fix for submatrix ordering
      x %= 254;
      y %= 256;
      if (subm == 3) {
        x = 253 - x;
  ...
  EUDRBController::EUDRBController(int id, int slot, int version)
      : m_id(id), m_version(version), m_mode(M_NONE), m_det(D_NONE),
        m_adcdelay(0), m_clkselect(0), m_pdrd(0), m_disabled(false),
        m_hasdbgreg(false), m_dbgreg(0) {
    m_vmes = VMEFactory::Create(slot << 27, 0x1000000);
    m_vmed = VMEFactory::Create(
        slot << 27, 0x1000000, VMEInterface::A32, VMEInterface::D32,
  ...
  std::string EUDRBController::Mode() const { return MODE_NAMES[m_mode]; }

  std::string EUDRBController::Det() const { return DET_NAMES[m_det]; }

  void EUDRBController::Configure(const eudaq::Configuration &param,
                                  int master) {
    std::cout << "Configuring board " << m_id << " as a "
              << (m_id == master ? "master" : "slave") << std::endl;
    m_disabled = false;
    std::string det = getpar(param, m_id, "Det", "");
    m_det = DetNum(det);
    std::string mode =
        getpar(param, m_id, "Mode", (m_det == D_MIMOSA26 ? "ZS2" : "ZS"));
    m_mode = ModeNum(mode);
    ...
    if (internaltiming)
      m_ctrlstat |= 0x2000;
    std::cout << "  Setting Control/Status   " << hexdec(m_ctrlstat)
              << std::endl;
    ...
    if (m_version < 3 && (marker1 >= 0 || marker2 >= 0)) {
      ...
      std::cout << "Setting board " << m_id << " markers to "
                << eudaq::hexdec(marker1, 2) << ", "
                << eudaq::hexdec(marker2, 2) << std::endl;
  ...
  std::string EUDRBController::PostConfigure(const eudaq::Configuration &param,
                                             int master) {
    std::string fname;
    ...
    if (m_mode == M_ZS) {
      ...
      if (fname == "") {
        if (mult != -1.0) {
          EUDAQ_WARN(
              "Multiplier being ignored because no pedestal file was given");
        }
        if (ped == -1)
          ped = 0;
        if (thresh == -1)
        ...
        if (thresh != -1 || ped != -1) {
          EUDAQ_WARN("Fixed pedestal values being ignored because a pedestal "
                     "file was given");
        }
        if (mult == -1.0)
          mult = 2.0;
        ...
        if (n != std::string::npos) {
          fname = std::string(fname, 0, n) + to_string(m_id) +
                  std::string(fname, n + 1);
        }
        if (fname[0] != '/' && fname[0] != '.')
          fname = "../pedestal/" + fname;
      }
      ...
      if (fname != "")
        msg += " from file " + fname + " * " + to_string(mult);
      else
        msg += " values = " + to_string(ped) + ", " + to_string(thresh);
      puts(msg.c_str());
      EUDAQ_INFO(msg);
      if (fname == "")
        fname = to_string(ped) + ":" + to_string(thresh);
    }
  ...
  bool EUDRBController::EventDataReady(double timeout) {
    if (m_version < 3)
      EUDAQ_THROW("EventDataReady only works wth FW >= 3");
    uint32_t i = 0;
    for (eudaq::Timer timer; timer.Seconds() < timeout; /**/) {
      ++i;
  ...
  int EUDRBController::EventDataReady_size(double timeout) {
    if (m_version >= 3)
      EUDAQ_THROW("EventDataReady_size only works wth FW < 3");
  ...
  int EUDRBController::ModeNum(const std::string &mode) {
    for (int i = 0; i < NUM_MODES; ++i) {
      if (eudaq::lcase(mode) == eudaq::lcase(MODE_NAMES[i]))
        return i;
    }
    return M_NONE;
  }
  ...
  int EUDRBController::DetNum(const std::string &det) {
    for (int i = 0; i < NUM_DETS; ++i) {
      if (eudaq::lcase(det) == eudaq::lcase(DET_NAMES[i]))
        return i;
    }
    return D_NONE;
  }
  ...
  EUDRBController::pedestal_t EUDRBController::GeneratePedestals(int thresh,
                                                                 int ped) {
    return pedestal_t(1UL << 20, thresh | (ped << 6));
  }

  EUDRBController::pedestal_t
  EUDRBController::ReadPedestals(const std::string &filename, float sigma,
                                 int det) {
    ...
    if (det == D_MIMOTEL) {
      ...
      EUDAQ_THROW("I don't know how to decode '" + to_string(det) +
                  "' pedestal files");
    }
    ...
    if (file.is_open()) {
      ...
      while (std::getline(file, line)) {
        ...
        if (flag)
          newdata32 = badpix; // mask bad pixels as good as you can (high thresh
                              // and very low ped)
  ...
  unsigned EUDRBController::UpdateDebugRegister() {
    m_dbgreg = m_vmes->Read(0x50);
    return GetDebugRegister();
  }
}
