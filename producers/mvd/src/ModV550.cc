...
void ModV550::GetFifo0(unsigned int OR, unsigned int V, int *ChNum,
                       int *ChData) {
...
void ModV550::GetFifo1(unsigned int OR, unsigned int V, int *ChNum,
                       int *ChData) {
...
void ModV550::Clear() { vme->Write((MClear), (unsigned short int)(0)); } // Clear
void ModV550::SetNumberOfChannels(unsigned short int ChN0,
                                  unsigned short int ChN1) {
...
void ModV550::SetInterrupt(unsigned short int IntLev,
                           unsigned short int StatID) {
...
void ModV550::SetPedestalThreshold(int adc, int chan, int ped, int thr) {
  ...
  vme32->Write(((adc ? PT1 : PT0) + chan * sizeof(unsigned)),
               (ped << 12) | thr);
...
void ModV550::GetPedestalThresholdFull(int adc, int chan, int *PedThr) {
  ...
  if (adc == 0)
    tmp = vme32->Read(PT0 + chan * sizeof(tmp), tmp);
  if (adc == 1)
    tmp = vme32->Read(PT1 + chan * sizeof(tmp), tmp);
