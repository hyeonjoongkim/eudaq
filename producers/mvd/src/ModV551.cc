...
void ModV551::Clear() { WriteVME16(SClear, 0); } // SetIRQLevel
void ModV551::SoftwareTrigger() { WriteVME16(STrigger, 0); } // SoftwareTrigger
...
void ModV551::SetInternalDelay(unsigned int ID) {
  ...
  if (ID)
    WriteVME16(Status, (tmp | 0x1));
  else
    WriteVME16(Status, (tmp & 0x3E));
...
void ModV551::SetVeto(unsigned int V) {
  ...
  unsigned int tmp = ReadVME16(Status) & 0x3F;
  if (V)
    WriteVME16(Status, (tmp | 0x2));
  else
    WriteVME16(Status, (tmp & 0x3D));
...
void ModV551::SetAutoTrigger(unsigned int AT) {
  ...
  unsigned int tmp = ReadVME16(Status) & 0x3F;
  if (AT)
    WriteVME16(Status, (tmp | 0x4));
  else
    WriteVME16(Status, (tmp & 0x3B));
...
void ModV551::SetTestMode(unsigned int TM) {
  ...
  if (TM)
    WriteVME16(Test, (tmp | 0x1));
  else
    WriteVME16(Test, (tmp & 0xE));
...
void ModV551::SetClockLevel(unsigned int CL) {
  ...
  unsigned int tmp = ReadVME16(Test) & 0xF;
  if (CL)
    WriteVME16(Test, (tmp | 0x2));
  else
    WriteVME16(Test, (tmp & 0xD));
...
void ModV551::SetShiftLevel(unsigned int SL) {
  ...
  unsigned int tmp = ReadVME16(Test) & 0xF;
  if (SL)
    WriteVME16(Test, (tmp | 0x4));
  else
    WriteVME16(Test, (tmp & 0xB));
...
void ModV551::SetTestPulsLevel(unsigned int PL) {
  ...
  unsigned int tmp = ReadVME16(Test) & 0xF;
  if (PL)
    WriteVME16(Test, (tmp | 0x8));
  else
    WriteVME16(Test, (tmp & 0x7));
...
void ModV551::SetTime(int t1, int t2, int t3, int t4, int t5) {
  ...
  if (t3 < 1 || t3 > 255 || t3 > t4) {
    printf(
        " !!!! Error v551: t3 value over range (1,255) t3 <= t4, t3=%d t4=%d\n",
        t3, t4);
    exit(1);
  }
...
void ModV551::SetVCAL(unsigned int VCAL) { WriteVME16(DAC, VCAL); } // SetVCAL
...
ModV551::~ModV551() { printf("ModV551B destructor is working \n"); } //~ModV551B
