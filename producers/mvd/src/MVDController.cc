...
MVDController::~MVDController() { Clear(); }
...
bool MVDController::Enabled(unsigned adc, unsigned subadc) {
  return adc < NumOfADC && subadc < MODV550_CHANNELS &&
         m_enabled[adc * MODV550_CHANNELS + subadc];
}
...
std::vector<unsigned int> MVDController::Read(unsigned adc, unsigned subadc) {
  if (adc >= NumOfADC)
    EUDAQ_THROW("Invalid ADC (" + to_string(adc) + " >= " +
                to_string(NumOfADC) + ")");
  if (subadc >= MODV550_CHANNELS)
    EUDAQ_THROW("Invalid SubADC (" + to_string(subadc) + " >= " +
                to_string(MODV550_CHANNELS) + ")");
...
bool MVDController::ActiveSequence() { return m_modv551->GetActiveSequence(); }
bool MVDController::DataReady() { return m_modv551->GetDataRedy(); }
...
void MVDController::ReadCalFile() {
  ...
  ADC_int = 0;
  int plane = 0;
  for (int i = 0; i < EventLength;
       i++) { // For all channel of strip detector  //EventLength
    ...
    m_modv550[NumOfCrate]->ModV550::SetPedestalThresholdFull(ADC_int, chan,
                                                             tmp);
    if (NumOfCrate == 1) {
...
void MVDController::CalibAutoTrig() {
    ...
    for (unsigned int i = 0; i < NumOfADC; i++) {
      ...
      for (int ADC_int = 0; ADC_int < 2; ADC_int++) {
        ...
        /*
                                  ...
                                  unsigned hits =
           m_modv550[i]->GetWordCounter(ADC_int);
           ...
           hits; chan++){
                                          ChTmp =
           m_modv550[i]->GetEvent(ADC_int);
                                          ChNum = (ChTmp >> 12) & 0xfff;
                                          ChData = (ChTmp & 0xfff);
                                          printf("ChNum= %d \n", ChNum);
                                          printf("ChData= %d \n", ChData);
                                          Ped[  ChNum + (SilcPointer*1280) ] +=
           ChData;
                                          Ped2[ ChNum + (SilcPointer*1280) ] +=
           ChData*ChData;
                                          Norm[ ChNum + (SilcPointer*1280) ] +=
           1;
                                          chan++;
                                  }
                        SilcPointer++;
  ...
  for(int tmp = 0; tmp < BuffSize; tmp++){
    ...
    if(Norm[tmp] > 1){
      ...
      Noi[tmp] = (int) ( sqrt(  (double)  ( (Norm[tmp]/(Norm[tmp] - 1))
*(Ped2[tmp] - Ped[tmp]*Ped[tmp]) )  )  );
    }
    else{
      Ped[tmp] = 0;
                Noi[tmp] = 0;
