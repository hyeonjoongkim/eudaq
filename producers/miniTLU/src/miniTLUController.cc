...
namespace tlu {
miniTLUController::miniTLUController(const std::string &connectionFilename,
                                     const std::string &deviceName)
    : m_hw(0), m_dataFromTLU(0), m_DACaddr(0), m_IDaddr(0) {

  std::cout << "Configuring from " << connectionFilename << " the device "
            << deviceName << std::endl;
  if (!m_hw) {
...
void miniTLUController::CheckEventFIFO() {
  m_nEvtInFIFO =
      miniTLUController::ReadRRegister("eventBuffer.EventFifoFillLevel");
  //   m_nEvtInFIFO = 2;
  if (m_nEvtInFIFO)
    std::cout << "words in FIFO : " << m_nEvtInFIFO << std::endl;
}

void miniTLUController::ReadEventFIFO() {
  if (m_nEvtInFIFO) {
    if (!(m_nEvtInFIFO))
      std::cout << "Warning odd words in fifo!" << std::endl;
    try {
      ValVector<uint32_t> fifoContent =
          m_hw->getNode("eventBuffer.EventFifoData").readBlock(m_nEvtInFIFO);
      m_hw->dispatch();
      if (fifoContent.valid()) {
        bool lowBits = false;
        uint64_t word = 0;
        //	std::cout << "Dump event FIFO" << std::endl;
        for (ValVector<uint32_t>::const_iterator i(fifoContent.begin());
             i != fifoContent.end(); ++i) {
...
void miniTLUController::InitializeI2C(char DACaddr, char IDaddr) {
  ...
  for (int myaddr = 0; myaddr < 128; myaddr++) {
    ...
    if (myaddr == DACaddr) {
      if (isConnected) {
        std::cout << "DAC at addr " << std::hex << myaddr << " is connected"
                  << std::endl;
        m_DACaddr = myaddr;
      } else {
        std::cout << "DAC at addr " << std::hex << DACaddr
                  << " is NOT connected" << std::endl;
      }
    } else if (myaddr == IDaddr) {
      if (isConnected) {
        std::cout << "ID at addr " << std::hex << myaddr << " is connected"
                  << std::endl;
        m_IDaddr = myaddr;
      } else {
        std::cout << "ID at addr " << std::hex << DACaddr << " is NOT connected"
                  << std::endl;
      }
    } else if (isConnected)
      std::cout << "Device " << std::hex << myaddr << " is connected"
                << std::endl;
    SetI2CTX(0x0);
    SetI2CCommand(0x50);
    while (I2CCommandIsDone()) {
  ...
  if (m_IDaddr) {
    ...
    for (unsigned char myaddr = 0xfa; myaddr > 0x0; myaddr++) {
      ...
      m_BoardID =
          ((((uint64_t)nibble) & 0xff) << ((0xff - myaddr) * 8)) | m_BoardID;
    }
    std::cout << "Unique ID : " << std::setw(12) << std::setfill('0')
              << m_BoardID << std::endl;
  }
}

void miniTLUController::WriteI2CChar(char deviceAddr, char memAddr,
                                     char value) {
...
void miniTLUController::WriteI2CCharArray(char deviceAddr, char memAddr,
...
void miniTLUController::SetDACValue(unsigned char channel, uint32_t value) {
  ...
  std::cout << "Setting DAC channel " << (unsigned int)channel << " = " << value
            << std::endl;
...
void miniTLUController::SetThresholdValue(unsigned char channel,
                                          float thresholdVoltage) {

  std::cout << "Setting threshold for channel " << (unsigned int)channel
            << " to " << thresholdVoltage << " Volts" << std::endl;
...
void miniTLUController::DumpEvents() {
  if (m_nEvtInFIFO)
    std::cout << "Called dump events. " << m_nEvtInFIFO
              << " 64 bit words in the buffer." << std::endl;
  for (int i = 0; i < m_nEvtInFIFO / 2;) {
    ...
    i++;
    std::cout << "Event number " << evtNumber << " type " << evtType
              << " input triggers 0: " << input0 << " 1: " << input1
              << " 2: " << input2 << " 3: " << input3 << std::endl;
    std::cout << "Timestamp " << timeStamp << " SC0[" << SC0 << "] SC1[" << SC1
              << "] SC2[" << SC2 << "] SC3[" << SC3 << "]" << std::endl;
  }
}
}
