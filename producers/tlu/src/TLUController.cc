...
//   on the PMT supply board (which changes the SET pin on the ADR130 chip, thus
//   doubling the reference voltage).
...
TLUController::TLUController(int errorhandler)
    : m_mask(0), m_vmask(0), m_amask(0), m_omask(0), m_ipsel(0xff),
      m_handshakemode(0x3F), m_triggerint(0), m_inhibit(true), m_vetostatus(0),
      m_fsmstatus(0), m_dutbusy(0), m_clockstat(0), m_dmastat(0),
      m_pmtvcntlmod(0), m_fsmstatusvalues(0), m_triggernum((unsigned)-1),
      m_timestamp(0), m_oldbuf(0), m_triggerBuffer(nullptr), m_particles(0),
      m_lasttime(0), m_errorhandler(errorhandler), m_version(0), m_addr(0),
  ...
  if (m_debug_level & TLU_DEBUG_CONFIG) {
    ...
    for (unsigned i = 0; i < NumCards; ++i) {
      std::cout << "DEBUG: Card " << i << ", ID = " << hexdec(CardIDs[i])
                ...
                << ", FPGAType = " << hexdec(FPGATypes[i]) << ", Possible TLU: "
...
bool TLUController::SetPMTVcntl(unsigned *values, double *gain_errors,
  ...
  for (int i = 0; i < TLU_PMTS; i++) {
    try {
      WriteI2C16((AD5316_HW_ADDR << 2) | m_addr->TLU_I2C_BUS_PMT_DAC, 0x08 >> i,
                 CalcPMTDACValue(target_voltages[i]));
    } catch (const eudaq::Exception &) {
      return false;
    }
  }
...
void TLUController::Update(bool timestamps) {
  ...
  if (timestamps) {
    ...
    trigger_buffer = m_triggerBuffer; // this is not multi thread save. but
                                      // since both are not multi thread save it
                                      // seems to be ok
  ...
  if (entries > 0) {
    ...
    for (unsigned i = 0; i < entries; ++i) {
      ...
      m_buffer.push_back(
          TLUEntry(timestamp_buffer ? timestamp_buffer[i] : NOTIMESTAMP, trig++,
...
uint64_t *TLUController::ReadBlock(unsigned entries) {
  ...
  for (int i = 0; i < count; ++i) { // loop round trying to read buffer
    ...
    if (m_debug_level & TLU_DEBUG_BLOCKREAD) {
      std::cout << "### Warning: detected error in block read. Trying a soft "
                   "correction by reading blocks again with padding (2nd time)"
                << std::endl;
      EUDAQ_WARN("### Warning: detected error in block read. Trying a soft "
                 "correction by reading blocks again with padding (2nd time)");
    }
    ...
    if (m_debug_level & TLU_DEBUG_BLOCKREAD) {
      std::cout << "### Warning: Re-read of block data failed to correct "
                   "problem. Will try to reset DMA buffer pointer" << std::endl;
...
unsigned TLUController::ReadBlockRaw(unsigned entries, unsigned buffer_offset) {
  ...
  // m_working_buffer );
  // Change syntax. Should be exactly the same but getting mysterious SEGFAULTs
  // so hack at random...
  result =
  ...
  // check to make sure that the first entry is zero ( which it should be unless
  // something has slipped )
  for (int tries = 0; tries < 4; ++tries) {
    if (m_working_buffer[tries][0] != 0) {
      std::cout << "### Warning: m_working_buffer[buf][0] != 0. This shouldn't "
                   "happen. buf = " << tries << std::endl;
  ...
  for (unsigned i = buffer_offset; i < entries + buffer_offset; ++i) {
    ...
    if ((m_working_buffer[1][i] == m_working_buffer[2][i]) ||
      ...
      std::cout << "### Warning: Uncorrectable data error in timestamp buffer. "
                   "location = " << i
  ...
  if (last_timestamp >= first_timestamp) {
    std::cout << "### Warning: First time-stamp from current buffer is older "
                 ...
                 "buf[0]) " << std::setw(8) << m_lasttime << "  " << m_oldbuf[0]
              << std::endl;
    num_uncorrectable_errors++;
  }
...
unsigned TLUController::ReadBlockSoftErrorCorrect(unsigned entries, bool pad) {
  ...
  if (m_debug_level & TLU_DEBUG_BLOCKREAD) {
    char *errmsg = 0;
    ZestSC1GetErrorMessage(static_cast<ZESTSC1_STATUS>(result), &errmsg);
    std::cout << (result == ZESTSC1_SUCCESS ? "" : "#### Warning (1st read): ")
              << errmsg << std::endl;
  }
  ...
  if (m_debug_level & TLU_DEBUG_BLOCKREAD) {
    char *errmsg = 0;
    ZestSC1GetErrorMessage(static_cast<ZESTSC1_STATUS>(result), &errmsg);
    std::cout << (result == ZESTSC1_SUCCESS ? "" : "#### Warning (2nd read): ")
              << errmsg << std::endl;
  }
  ...
  if (m_debug_level & TLU_DEBUG_BLOCKREAD) {
    std::cout << "#### Number of errors reported by ReadBlockRaw after rsync = "
              << num_errors << std::endl;
...
unsigned TLUController::ResetBlockRead(unsigned entries) {
  ...
  std::cout
      << "### Read back (after dropping reset line) INITIATE_READOUT_ADDRESS: "
      << new_dma_status << std::endl;
...
void TLUController::SetLEDs(const std::vector<TLU_LEDs> &leds) {
  ...
  if (m_version == 1) {
    ...
    // std::cout << "\n  dut=" << hexdec(dutled) << ", lemo=" << hexdec(lemoled)
    // << std::endl;
