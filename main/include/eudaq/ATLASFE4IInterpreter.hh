...
protected:
  ...
  inline bool is_dr(uint X) const {
    ...
    return (
        ((dr_col_msk & X) <= dr_max_col) && ((dr_col_msk & X) >= dr_min_col) &&
        ((dr_row_msk & X) <= dr_max_row) && ((dr_row_msk & X) >= dr_min_row));
  }
  ...
  static const uint tr_wrd_hdr = 0x00F80000; // tolerant to 1-bit flips and not
                                             // equal to control/comma symbols
  ...
  static const uint tr_err_msk = 0x00001F00;  // error code: bit 0: wrong number
                                              // of dh, bit 1 service record
                                              // recieved
