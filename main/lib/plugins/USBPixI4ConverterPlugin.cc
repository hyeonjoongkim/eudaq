...
namespace eudaq {
  ...
  /** Base converter class for the conversion of EUDAQ data into
   * StandardEvent/LCIO data format
  ...
  class USBPixI4ConverterBase
      : public ATLASFEI4Interpreter<dh_lv1id_msk, dh_bcid_msk> {
  ...
  protected:
    ...
    USBPixI4ConverterBase(const std::string &event_type)
        : advancedConfig(false), EVENT_TYPE(event_type) {}
      ...
      if (moduleConfig.size() > 0) {
        ...
        for (int value : moduleCount) {
          for (int j = 1; j <= value; j++) {
            if (j > 4) {
              std::cerr << "ERROR: USBPixI4ConverterPlugin: Your setup "
                           "contains chips which are larger than 4!"
                        << std::endl;
              throw std::runtime_error("USBPixI4ConverterPlugin: Your setup "
                                       "contains chips which are larger than "
                                       "4!");
            }
    ...
    unsigned getTrigger(const std::vector<unsigned char> &data) const {
      ...
      unsigned int trigger_number =
          this->get_tr_no_2(Trigger_word1, Trigger_word2);
    ...
    StandardPlane ConvertPlane(const std::vector<unsigned char> &data,
      ...
      plane.SetSizeZS((CHIP_MAX_COL_NORM + 1) * colMult,
                      (CHIP_MAX_ROW_NORM + 1) * rowMult, 0, consecutive_lvl1,
    ...
    void transformChipsToModule(unsigned int &col, unsigned int &row,
                                int chip) const {
      switch (chip) {
      default:
        break;
    ...
    inline unsigned int getWord(const std::vector<unsigned char> &data,
                                size_t index) const {
      return (((unsigned int)data[index + 3]) << 24) |
             (((unsigned int)data[index + 2]) << 16) |
             (((unsigned int)data[index + 1]) << 8) | (unsigned int)data[index];
    }
  };
  ...
  class USBPixI4ConverterPlugin
      : public DataConverterPlugin,
        public USBPixI4ConverterBase<dh_lv1id_msk, dh_bcid_msk> {
  ...
  public:
    ...
    virtual bool GetStandardSubEvent(StandardEvent &sev,
      ...
      for (size_t i = 0; i < ev_raw.NumBlocks(); ++i) {
        if (this->advancedConfig) {
          sev.AddPlane(
              this->ConvertPlane(ev_raw.GetBlock(i), this->moduleConfig.at(i)));
        } else {
    ...
    virtual bool GetLCIOSubEvent(lcio::LCEvent &lcioEvent,
      ...
      // this is an event as we sent from Producer, needs to be converted to
      // concrete type RawDataEvent
      ...
      if (this->advancedConfig)
        previousSensorID = this->moduleConfig.at(0) + chip_id_offset - 1;
      else
        previousSensorID =
            ev_raw.GetID(0) + chip_id_offset + this->first_sensor_id;
      ...
      for (size_t chip = 0; chip < ev_raw.NumBlocks(); ++chip) {
        ...
        if (this->advancedConfig) {
          ...
          sensorID =
              ev_raw.GetID(chip) + chip_id_offset + this->first_sensor_id;
        }
        ...
        if (previousSensorID != sensorID) {
          ...
          std::unique_ptr<lcio::TrackerDataImpl> newZsFrame(
        ...
        for (size_t i = 0; i < buffer.size() - 4; i += 4) {
          ...
          if (this->is_dh(Word)) {
            ...
            if (this->getHitData(Word, false, Col, Row, ToT)) {
              if (this->advancedConfig)
                this->transformChipsToModule(Col, Row,
                                             this->moduleIndex.at(chip));
  ...
  protected:
    USBPixI4ConverterPlugin(const std::string &event_type)
        : DataConverterPlugin(event_type),
          USBPixI4ConverterBase<dh_lv1id_msk, dh_bcid_msk>(event_type),
          chip_id_offset(20) {}
  };
  ...
  private:
    ...
    USBPixFEI4AConverter()
        : USBPixI4ConverterPlugin<0x00007F00, 0x000000FF>(USBPIX_FEI4A_NAME){};
  ...
  private:
    USBPixFEI4BConverter()
        : USBPixI4ConverterPlugin<0x00007C00, 0x000003FF>(USBPIX_FEI4B_NAME){};
