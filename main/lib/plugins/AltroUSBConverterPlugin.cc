...
/** A helper class to read the byte sequence of big endian 32 bit data as 10 bit
 * or  40 bit data.
...
uint64_t UCharAltroUSBVec::Get40bitWord(unsigned int index40bit) const {
  ...
  size_t index8bit =
      index40bit * 8; // there are 8 bytes (64 bits) per 40 bit word;

  uint64_t retval = static_cast<uint64_t>(_bytedata[index8bit]) |
                    static_cast<uint64_t>(_bytedata[index8bit + 1]) << 8 |
                    static_cast<uint64_t>(_bytedata[index8bit + 2] & 0xf)
                        << 16 |
...
unsigned short UCharAltroUSBVec::Get10bitWord(unsigned int index10bit) const {
  // There are two 10 bit words stored in a 32 bit word. For each it is easiest
  // to start at the beginning
  // of the 32 bit word. So the 8bit-index is the beginning of the 32 bit word
  // in both cases.
  ...
  if ((index10bit % 2) == 0) // 10 bit index is even
  {
    ...
    retval = _bytedata[index8bit] | ((_bytedata[index8bit + 1] & 0x03) << 8);
...
#if USE_LCIO
bool AltroUSBConverterPlugin::GetLCIOSubEvent(
  ...
  try {
    ...
    for (size_t block = 0; block < rawdataevent.NumBlocks(); block++) {
      ...
      std::cout << "Altro block has " << altrodatavec.Size() << " 40bit words"
                << std::endl;
      std::cout << "Last altro word is 0x" << std::hex
                << altrodatavec.Get40bitWord(altrotrailerposition) << std::dec
                << std::endl;
      ...
      while (altrotrailerposition > 0) {
        ...
        uint64_t altrotrailer = altrodatavec.Get40bitWord(altrotrailerposition);

        // bool channel_is_broken = false;
        // test if we realy have the altro trailer word
        if ((altrotrailer & 0xFFFC00F000ULL) != 0xAAA800A000ULL) {
          std::cout << "Altro trailer is 0x" << std::hex << altrotrailer
                    << ", signature " << (altrotrailer & 0xFFFC00F000ULL)
                    << std::dec << std::endl;
          DumpDataBlock(altrodatavec);
          throw BadDataBlockException("Invalid Altro trailer word");
        }

        // if the channel is okn we continue with reading the number of 10bit
        // words
        ...
        while (index10bit > (altrotrailerposition * 4) -
          ...
          if (index10bit == previous_index10bit) {
            std::cout << "trying to read 10bit index " << index10bit << " twice"
                      << std::endl;
            std::cout << std::hex << "altrotrailer " << altrotrailer << std::dec
                      << std::endl;
            throw BadDataBlockException(
...
#else  // if USE_LCIO
bool AltroUSBConverterPlugin::GetLCIOSubEvent(lcio::LCEvent &,
                                              eudaq::Event const &) const {
  return false;
}
#endif // IF USE_LCIO

bool AltroUSBConverterPlugin::GetStandardSubEvent(StandardEvent &,
                                                  eudaq::Event const &) const {
...
void AltroUSBConverterPlugin::DumpDataBlock(
    UCharAltroUSBVec const &rawdatavec) const {
  for (size_t i = 0; i < rawdatavec.Size(); i++) {
    std::cout << i << "\t 0x" << std::hex << rawdatavec.Get40bitWord(i)
              << "\t 0x" << rawdatavec.Get10bitWord(4 * i) << "\t 0x"
              << rawdatavec.Get10bitWord(4 * i + 1) << "\t 0x"
              << rawdatavec.Get10bitWord(4 * i + 2) << "\t 0x"
              << rawdatavec.Get10bitWord(4 * i + 3) << std::dec << std::endl;
  }
}
