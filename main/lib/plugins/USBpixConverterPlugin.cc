...
namespace eudaq {
  ...
  public:
    ...
    bool isEventValid(const std::vector<unsigned char> &data) const {
      ...
      unsigned Trigger_word = (((unsigned int)data[i + 3]) << 24) |
                              (((unsigned int)data[i + 2]) << 16) |
                              (((unsigned int)data[i + 1]) << 8) |
                              (unsigned int)data[i];
      if (Trigger_word == (unsigned)-1)
        return false;

      if (TRIGGER_WORD_ERROR_MACRO(Trigger_word) != 0)
        return false;

      // ceck data consistency
      unsigned int eoe_found = 0;
      for (unsigned int i = 0; i < data.size() - 4; i += 4) {
        unsigned word = (((unsigned int)data[i + 3]) << 24) |
                        (((unsigned int)data[i + 2]) << 16) |
                        (((unsigned int)data[i + 1]) << 8) |
                        (unsigned int)data[i];
        if ((TRIGGER_WORD_HEADER_MACRO(word) == 0) &&
            (HEADER_MACRO(word) == 1) &&
            ((FLAG_MACRO(word) & FLAG_WO_STATUS) == FLAG_WO_STATUS)) {
          eoe_found++;
        }
      }

      if (eoe_found != consecutive_lvl1)
        return false;
      else
        return true;
    }
    ...
    bool getHitData(unsigned int &Word, unsigned int &Col, unsigned int &Row,
                    unsigned int &ToT) const {

      if ((FLAG_MACRO(Word) & FLAG_WO_STATUS) == FLAG_WO_STATUS)
        return false; // EoE word; skip for the timebeing
      if (TRIGGER_WORD_HEADER_MACRO(Word) == 1)
        return false; // trigger word
      ...
      // std::cout << "Event: (" << getBoardID (id) << " - " <<
      // getBoardOrientation(id) << ") " << trigger_number << ": " << Row << ":"
      // << Col << ":" << ToT << std::endl;
    ...
    StandardPlane ConvertPlane(const std::vector<unsigned char> &data,
      ...
      // Get Events
      for (unsigned int i = 0; i < data.size() - 4; i += 4) {
        unsigned int Word = (((unsigned int)data[i + 3]) << 24) |
                            (((unsigned int)data[i + 2]) << 16) |
                            (((unsigned int)data[i + 1]) << 8) |
                            (unsigned int)data[i];

        if ((HEADER_MACRO(Word) == 1) &&
            ((FLAG_MACRO(Word) & FLAG_WO_STATUS) == FLAG_WO_STATUS)) {
          lvl1++;
  ...
  class USBPixConverterPlugin : public DataConverterPlugin,
                                public USBpixConverterBase {
  ...
  public:
    ...
    virtual bool GetLCIOSubEvent(lcio::LCEvent &lcioEvent,
      ...
      // std::cout << "getlciosubevent (I3) event " <<
      ...
      for (size_t chip = 0; chip < ev_raw.NumBlocks(); ++chip) {
        ...
        if (lcioEvent.getEventNumber() == 0) {
          eutelescope::EUTelPixelDetector *currentDetector =
              new eutelescope::EUTelAPIXMCDetector(1);
        ...
        zsDataEncoder["sensorID"] =
            ev_raw.GetID(chip) + chip_id_offset + first_sensor_id;
        ...
        for (unsigned int i = 0; i < buffer.size() - 4; i += 4) {
          unsigned int Word = (((unsigned int)buffer[i + 3]) << 24) |
  ...
  private:
    ...
    USBPixConverterPlugin() : DataConverterPlugin(EVENT_TYPE) {
      consecutive_lvl1 = 16;
    }
