      ...
      if (ZEROSUPP == 0) {
        ...
        for (unsigned irowdcd = 0; irowdcd < plane.YSize() / 2; irowdcd++) {
          ...
          for (unsigned icoldcd = 0; icoldcd < plane.XSize() * 2; icoldcd++) {
            ...
            //     data=%f\n",icol,irow,d);
            //	    if(d>-0.1 && d<0.1)  printf("QQQ ipix=%d icol=%d,irow=%d
            //d=%f  DATA1=%f \n",i, icol,irow,
...
bool DEPFETConverterBase::ConvertLCIO(lcio::LCEvent &result,
  ...
  CellIDEncoder<TrackerRawDataImpl> rawDataEncoder(
      eutelescope::EUTELESCOPE::MATRIXDEFAULTENCODING, rawDataCollection.get());
  CellIDEncoder<TrackerDataImpl> zsDataEncoder(
      eutelescope::EUTELESCOPE::ZSDATADEFAULTENCODING, zsDataCollection.get());
    ...
    if (ZEROSUPP == 0) {
      ...
      for (size_t i = 0; i < nPixel; i++) {
        printf("EvSize=%d iPixel =%d DATA=%d  icol=%d irow=%d  \n", (int)nPixel,
               (int)i, (signed short)plane.GetPixel(i, 0),
