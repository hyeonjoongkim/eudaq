...
public:
  virtual bool GetStandardSubEvent(StandardEvent &sev, const Event &ev) const {
    ...
    for (int i = 0; i < event.plane_size(); i++) {
      ...
      // TODO: Add somethign smarted then this for raw data (need protocol
      // change?)

      if (pb_flags.zs())
        plane.SetSizeZS(pb_plane.xsize(), pb_plane.ysize(), 0,
                        pb_plane.frame_size(), flags);
      else
        plane.SetSizeRaw(pb_plane.xsize(), pb_plane.ysize(),
                         pb_plane.frame_size(), flags);
      ...
      for (int iframe = 0; iframe < pb_plane.frame_size(); iframe++) {
        ...
        for (int i = 0; i < pb_frame.pixel_size(); i++) {
          ...
          plane.PushPixel(pb_pix.x(), pb_pix.y(), pb_pix.val(), pb_pix.pivot(),
                          iframe);
        }
      }
...
private:
  GenericConverterPlugin() : DataConverterPlugin(EVENT_TYPE) {}
