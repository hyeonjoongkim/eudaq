...
namespace eudaq {
  ...
  private:
    const unsigned char *GetRawDataChecked(const Event &,
  ...
  bool Mupix3ConverterPlugin::GetStandardSubEvent(StandardEvent &dest,
    ...
    // empty data
    if (data == NULL) {
      EUDAQ_WARN("received event " + eudaq::to_string(source) +
                 " with no data blocks.");

      StandardPlane plane(MUPIX3_SENSOR_ID, MUPIX3_EVENT_TYPE,
                          MUPIX3_SENSOR_TYPE);
  ...
  bool
  Mupix3ConverterPlugin::GetLCIOSubEvent(lcio::LCEvent &dest,
  ...
  const unsigned char *
  Mupix3ConverterPlugin::GetRawDataChecked(const Event &ev,
                                           unsigned &n_hits) const {
    const RawDataEvent &raw = dynamic_cast<const RawDataEvent &>(ev);

    if (raw.NumBlocks() != 1) {
      EUDAQ_WARN("event " + to_string(raw.GetEventNumber()) +
  ...
  unsigned
  Mupix3ConverterPlugin::ExtractTriggerId(const unsigned char *data) const {
    return 0xFFFF & eudaq::getlittleendian<uint32_t>(data + 8);
  }

  // extract a single hit from the raw data
  void Mupix3ConverterPlugin::ExtractHit(const unsigned char *data,
