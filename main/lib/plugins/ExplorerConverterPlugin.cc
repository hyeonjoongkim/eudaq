...
public:
  ...
  virtual bool GetStandardSubEvent(StandardEvent &sev, const Event &ev) const {
    ...
    // array to save the iterator starting point of each frame ordered by frame
    // number 16+1
    ...
    framepos[(int)*it] = make_pair(
        it + 1, FRAME_LENGTH - 12); // readout the frame count should be 0 here
    it++;
      ...
      try {
        ...
        for (unsigned i = 0; i < plane.XSize(); i++)
          for (unsigned j = 0; j < plane.YSize(); j++) {
            ...
            //                        cerr << plane.GetPixel(j*plane.XSize()+i)
            //                        << endl;
          }
...
private:
  ...
  ExplorerConverterPlugin()
      : DataConverterPlugin(EVENT_TYPE), m_ExNum(0), m_PedMeas(0), m_UsePed(0),
        m_PedFile(""), m_UseThr(0), m_Thr(0), m_noe(0) {}
