...
namespace eudaq {
  ...
  public:
    ...
    virtual void Initialize(const Event &bore,
      ...
      m_PedFile =
          bore.GetTag<string>("PedestalFileToUse", "../data/pedestal0.ped");
      m_UseThr = bore.GetTag<bool>("UseThreshold", true);
      m_Thr = bore.GetTag<unsigned int>("Threshold", 10);

      cout << endl
           << "INITIALIZE:" << endl
           << m_ExNum << "\t" << (m_PedMeas == true ? "1" : "0") << "\t"
           << (m_UsePed == true ? "1" : "0") << "\t" << m_PedFile << "\t"
           << (m_UseThr == true ? "1" : "0") << "\t" << m_Thr << endl;
      ...
      if (!m_PedMeas) { // if not pedestal measurement, perform GC
        ...
        while (vbb_file.good() && !vbb_found) {
          vbb_file >> rn >> vbb >>
              gcorr_file; // run number, V_bb, path to g_corr.root file
          if (rn == bore.GetRunNumber())
            vbb_found = true;
        }
        vbb_file.close();
        if (!vbb_found)
          cerr << "\nERROR: VBB value not found for this run!!! (run "
               << bore.GetRunNumber() << ")\n\n";
        else {
          ...
          for (int ich = 0; ich < 4; ++ich) {
            ...
            gCorr_90_mem[ich]->SetChan(
                ich); // channel of the readout (0 = E4, 3 = E1!)
            gCorr_90_mem[ich]->SetVrstRef(
                0.7);                         // current standard reference gain
            ...
            gCorr_60_mem[ich]->SetChan(
                ich); // channel of the readout (0 = E4, 3 = E1!)
            gCorr_60_mem[ich]->SetVrstRef(
                0.7);                         // current standard reference gain
    ...
    virtual unsigned GetTriggerID(const Event &ev) const {
      ...
      if (const RawDataEvent *rev = dynamic_cast<const RawDataEvent *>(&ev)) {
        ...
        if (size_ADC < offset || size_HLVDS < offset)
          return (unsigned)(-1);
        unsigned int pos_HLVDS = size_HLVDS - offset;
        unsigned int pos_ADC = size_ADC - offset;
        unsigned short id_HLVDS =
            getbigendian<unsigned short>(&rev->GetBlock(0)[pos_HLVDS]);
        unsigned short id_ADC =
            getbigendian<unsigned short>(&rev->GetBlock(1)[pos_ADC]);
        if (id_ADC == id_HLVDS)
          return (unsigned)id_ADC;
        else {
    ...
    virtual bool GetStandardSubEvent(StandardEvent &sev,
      ...
      if (ev.IsEORE() && m_PedMeas) { // generate output pedestals and noise
        ...
        // average pedestal and noise (calculated separately for each pitch and
        // chip)
        double avped[m_nExplorers * 2] = {0., 0., 0., 0., 0.,
                                          0., 0., 0.}; // what a nice definition
                                                       // of a variable... lets
                                                       // hope that m_nExplorers
                                                       // never changes...
        double avnoise[m_nExplorers * 2] = {0., 0., 0., 0., 0., 0., 0., 0.};
        // containers for pedestal and noise which are going to be written to
        // disk
        vector<double> peds;
        vector<double> noise;
        ...
        for (unsigned int ievt = 0; ievt < cds.size(); ++ievt) {
          for (unsigned int ipix = 0; ipix < m_Ped.size(); ++ipix) {
            if (ievt == 0) {
              ...
              noise.push_back(((double)cds[ievt][ipix] - peds[ipix]) *
                              ((double)cds[ievt][ipix] - peds[ipix]));
            } else
              noise[ipix] += ((double)cds[ievt][ipix] - peds[ipix]) *
                             ((double)cds[ievt][ipix] - peds[ipix]);
          }
        }
        ...
        for (int i = 0; i < m_nExplorers * 2; ++i) {
          double div =
              (i % 2 == 0)
                  ? 1. / 8100.
                  : 1. / 3600.; // nice practice of shadowing a variable
          avped[i] *= div;
          avnoise[i] *= div;
        }
        ...
        cout << "Chip\t20x20\t30x30" << endl;
        for (int i = 0; i < m_nExplorers; ++i) {
          cout << i << '\t' << avnoise[2 * i] << '\t' << avnoise[2 * i + 1]
               << endl;
        }
      ...
      if (data.size() - 4 != unpack_fh(it, EVENT_LENGTH) ||
        ...
        std::cout << "\tEvent Length not correct!" << std::endl
                  ...
                  << " Seen: " << data.size() - 4 << " Correct Value: 140699"
                  << std::endl;
        return false;
      }
      ...
      if (FRAME_LENGTH == 9) {
        printf("\nWRONG\n");
        return false;
      } // discard events with EoE-Frame as First frame
      ...
      while (it != data.end()) {
        ...
        if (unpack_fh(it, FRAME_LENGTH) == 9) { // EOE expection
          if (unpack_fn(it, evn) == EVENT_NUM) {
            ...
            nframes++;
            if (15 != *it)
              cout << "Frame Counter ERROR: No Match of FrmCnt Max in EOE-Frame"
                   << endl;
            it++;
          } else {
            cout << "EOE ERROR:" << endl
          ...
          if ((int)*it <= 0 || (int)*it > 16) {
            cout << "Frame Counter ERROR:\n"
                 << "\tFramecount not possible! <1.|==1.|>16" << endl
                 << "\tDropping Event" << endl;
            return false;
          }
          framepos[(int)*it] =
      ...
      // CMC variables
      short cds_array[4][2][9][30 * 30]; // [channel][matrix size - 90=0 or
                                         // 60=1][sector][pixel]
      ...
      if (m_UsePed) { // set pedestal value in case of a Measurement using
                      // Pedestal
        ...
        if (pedestal.size() == 0)
          std::cout << "Failed opening pedestal file!" << std::endl;
        if (pedestal.size() != 4 * (60 * 60 + 90 * 90))
          std::cout << "Wrong pedestal data size!" << pedestal.size()
                    << " instead of " << 4 * (60 * 60 + 90 * 90) << std::endl;
      }
      ...
      vector<unsigned char>::iterator
          end; // iterator to indicate the end of datafr
      ...
      // read pixel values from file, save them in vector 'raw_values' and
      // 'cds_array' for cmc processing
        ...
        for (int j = 0; j < matrix_size; j++) { // y
          for (unsigned int k = 0; k < 2; k++) { // mem
            ...
            for (unsigned int l = 0; l < 4; l++) { // fill matrixes
              adccInt = (short)((buffer >> (12 * l)) &
                                0x0FFF); // mask + shift of Data value
              raw_values.push_back(adccInt); // fill vector containing raw
                                             // values
              ...
              if (matrix_size == 90) {
                ...
                if (k == 0) {
                  cds_array[l][0][sec][(j % 30) * 30 + (i % 30)] =
                      adccInt; // set to MEM1
                } else {
                  cds_array[l][0][sec][(j % 30) * 30 + (i % 30)] -=
                      adccInt; // subtract MEM2
                }
              } else { // matrix size is 60
                short sec = 3 * floor(i / 20) + floor(j / 20);
                if (k == 0) {
                  cds_array[l][1][sec][(j % 20) * 20 + (i % 20)] =
                      adccInt; // set to MEM1
                } else {
                  cds_array[l][1][sec][(j % 20) * 20 + (i % 20)] -=
                      adccInt; // subtract MEM2
          ...
          if (i == matrix_size - 1 && j == matrix_size - 1 &&
              matrix_size != (int)height2) { // change matrix size
            i = 0;
            j = -1;
      ...
      for (int ich = 0; ich < 4; ++ich) {   // channels
        for (int ips = 0; ips < 2; ++ips) { // pixel size - 90 and 60 matrices
          for (int isec = 0; isec < 9; ++isec) { // sectors
            ...
            std::sort(cds_array[ich][ips][isec],
                      cds_array[ich][ips][isec] + nwm);
            ...
            for (i = nwm - trunc; i < nwm; ++i)
              sec_wm[ich][ips][isec] +=
                  cds_array[ich][ips][isec][nwm - trunc - 1];
      ...
      // correct the values for the CM and do whatever needs to be done with
      // them
      type = 0;
        ...
        for (int j = 0; j < matrix_size; j++) { // y
          for (unsigned int k = 0; k < 2; k++) { // mem
            for (unsigned int l = 0; l < 4; l++) { // fill matrixes
              ...
              // zero supression
              if (m_UsePed && k == 0 && pedestal.size() > 0) {
                adcc -= *pedit;
                pedit++;
              } // iterate the pedestal values
              // MEM1 - pedestal
              if (m_UseThr && k == 0)
                adcc -= m_Thr;
              ...
              // if ( i == 39 && j==38 && l ==2) cout << adcc << endl; // old
              // debug line
              planes[2 * l + type].SetPixel(j + i * matrix_size, i, j, adcc,
                                            k); // add value to StandardPlane
      ...
      if (m_PedMeas) { // sum up pedestals in a vector and sum up square of
                       // pedestals
      ...
      it_HLVDS += 20; // 4 byte event size, 4 frame size, 12 byte frame header,
                      // then the trailer startrs
...
///////////////////////////////////////////////////////////
    ...
    virtual bool GetLCIOSubEvent(lcio::LCEvent &lev,
      ...
      for (unsigned int n = 0; n < nplanes;
        ...
        lcio::TrackerRawDataImpl *explorerlciodata =
        ...
        lcio::LCCollectionVec *explorerTrackerDataCollection =
            new lcio::LCCollectionVec(lcio::LCIO::TRACKERDATA);
        try {
          //                    cerr << "try" << endl;
          // if the collection is already existing add the current data
          explorerCollection = dynamic_cast<lcio::LCCollectionVec *>(
              lev.getCollection("ExplorerRawData"));
          explorerCollection->addElement(explorerlciodata);
          explorerTrackerDataCollection = dynamic_cast<lcio::LCCollectionVec *>(
              lev.getCollection("ExplorerTrackerData"));
          CellIDEncoder<lcio::TrackerDataImpl> explorerEncoder(
              eutelescope::EUTELESCOPE::MATRIXDEFAULTENCODING,
              explorerTrackerDataCollection);
          ...
          for (unsigned i = 0; i < plane.XSize(); i++)
            for (unsigned j = 0; j < plane.YSize(); j++) {
              ...
              //                        cerr <<
              //                        plane.GetPixel(j*plane.XSize()+i) <<
              //                        endl;
            }
          ...
          explorerTrackerDataCollection->addElement(Frame);
          lev.addCollection(explorerTrackerDataCollection,
                            "ExplorerTrackerData");
        }
      }
  ...
  private:
    ...
    ExplorerConverterPlugin()
        : DataConverterPlugin(EVENT_TYPE), m_ExNum(0), m_PedMeas(0),
          m_UsePed(0), m_PedFile(""), m_UseThr(0), m_Thr(0), m_noe(0) {}
    ...
    mutable unsigned int m_noe; //# of converted events and with the events the
                                //added pedestal values
