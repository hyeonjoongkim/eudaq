...
// virtual void GetLCIORunHeader(lcio::LCRunHeader & header, eudaq::Event const
// & bore, eudaq::Configuration const & conf) const {
//   return ConvertLCIOHeader(header, bore, conf);
// }

// virtual bool GetLCIOSubEvent(lcio::LCEvent & lcioEvent, const Event &
// eudaqEvent) const {
...
// void ConvertLCIOHeader(lcio::LCRunHeader & header, eudaq::Event const &
// /*bore*/, eudaq::Configuration const & /*conf*/) const {
...
//     auto_ptr< lcio::LCCollectionVec > rawDataCollection ( new
//     lcio::LCCollectionVec (lcio::LCIO::TRACKERRAWDATA) ) ;
//     auto_ptr< lcio::LCCollectionVec > zsDataCollection ( new
//     lcio::LCCollectionVec (lcio::LCIO::TRACKERDATA) ) ;
...
// 	      const RawDataEvent * rev = dynamic_cast<const RawDataEvent *>
// (&source);
// 	      //cout << "[Number of blocks] " << rev->NumBlocks() << endl;
...
//   	      // Note X and Y are swapped - for 2010 TB DEPFET module was
...
//         } catch (...) {
//           timepixSetupCollection = new LCCollectionVec(
//           lcio::LCIO::LCGENERICOBJECT );
//         }
  ...
  bool MimosaConverterPlugin::ReadNextInt() {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if (ch[7 - i] == '0') {
        ...
        tmpVal = 9;
      } else if (ch[7 - i] == 'A' || ch[7 - i] == 'a') {
        tmpVal = 10;
      } else if (ch[7 - i] == 'B' || ch[7 - i] == 'b') {
        tmpVal = 11;
      } else if (ch[7 - i] == 'C' || ch[7 - i] == 'c') {
        tmpVal = 12;
      } else if (ch[7 - i] == 'D' || ch[7 - i] == 'd') {
        tmpVal = 13;
      } else if (ch[7 - i] == 'E' || ch[7 - i] == 'e') {
        tmpVal = 14;
      } else if (ch[7 - i] == 'F' || ch[7 - i] == 'f') {
        tmpVal = 15;
      }
  ...
  bool MimosaConverterPlugin::ReadFrame(short data[][kColPerChip]) {
    ...
    for (short row = 0; row < kRowPerChip; row++) {
      ...
      for (short i = 0; i < 5; i++) {
        ...
        unsigned int word = fDataChar1 + (fDataChar2 << 8) +
                            (fDataChar3 << 16) + (fDataChar4 << 24);
        data[row][col] = ((word << 2 * i) + tmpbits) & 0x000003FF;
        col++;
        data[row][col] = (word >> (10 - 2 * i)) & 0x000003FF;
        col++;
        data[row][col] = (word >> (20 - 2 * i)) & 0x000003FF;
        col++;
