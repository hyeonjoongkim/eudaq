...
namespace eudaq {
  ...
  unsigned Mupix4ConverterPlugin::GetTriggerID(const Event &e) const {
    ...
    if (e.IsBORE() || e.IsEORE())
      return (unsigned)(-1);
    if (!(data = Mupix4DataProxy::from_event(e)))
      return (unsigned)(-1);
   ...
   */
  bool Mupix4ConverterPlugin::GetStandardSubEvent(StandardEvent &dest,
    ...
    StandardPlane plane(MUPIX4_SENSOR_ID, MUPIX4_EVENT_TYPE,
                        MUPIX4_SENSOR_TYPE);
    plane.SetSizeZS(MUPIX4_SENSOR_NUM_COLS, MUPIX4_SENSOR_NUM_ROWS,
                    data.num_hits());
  ...
  bool
  Mupix4ConverterPlugin::GetLCIOSubEvent(lcio::LCEvent &dest,
    ...
    for (unsigned i = 0; i < data.num_hits(); ++i) {
      EUTelGenericSparsePixel pixel(data.hit_row(i), data.hit_col(i),
                                    MUPIX4_SENSOR_BINARY_SIGNAL);
