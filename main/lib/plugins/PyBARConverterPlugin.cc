...
public:
  ...
  unsigned getTrigger(const std::vector<unsigned char> &data) const {
    ...
    // (((unsigned int)data[i + 2]) << 16) | (((unsigned int)data[i + 1]) << 8)
    // | (unsigned int)data[i];
...
public:
  ...
  virtual bool GetLCIOSubEvent(lcio::LCEvent &lcioEvent,
    ...
    // std::cout << "getlciosubevent (I4) event " << eudaqEvent.GetEventNumber()
    // << " | " << GetTriggerID(eudaqEvent) << std::endl;
    ...
    for (size_t chip = 0; chip < ev_raw.NumBlocks(); ++chip) {
      ...
      zsDataEncoder["sensorID"] =
          ev_raw.GetID(chip) + chip_id_offset + first_sensor_id; // formerly 14
