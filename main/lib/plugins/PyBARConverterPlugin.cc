...
namespace eudaq {
  ...
  static const unsigned int CHIP_MAX_ROW_NORM =
      CHIP_MAX_ROW - CHIP_MIN_ROW; // Maximum ROW normalized (starting with 0)
  ...
  public:
    ...
    int getCountBoards() { return count_boards; }

    std::vector<unsigned int> getBoardIDs() { return board_ids; }

    unsigned int getBoardID(unsigned int board_index) const {
      if (board_index >= board_ids.size())
        return (unsigned)-1;
    ...
    bool isEventValid(const std::vector<unsigned char> &data) const {
      ...
      for (unsigned int i = 4; i < data.size();
           i += 4) { // 8->4 trigger from pybar is 32bits
        unsigned word = (((unsigned int)data[i]) << 24) |
                        (((unsigned int)data[i + 1]) << 16) |
                        (((unsigned int)data[i + 2]) << 8) |
                        (unsigned int)data[i + 3];
        // printf("isEventValid word=%08x,masked=%08x,
        // %d\n",word,(PYBAR_DATA_HEADER_MASK &
        // word),int(PYBAR_DATA_HEADER_MACRO(word)));
    ...
    unsigned getTrigger(const std::vector<unsigned char> &data) const {
      ...
      //}
      // printf("\n");
      // unsigned int i = data.size() - 4; // 8->4 hirono //splitted in 2x 32bit
      // words
      unsigned Trigger_word1 =
          (((unsigned int)data[0]) << 24) | (((unsigned int)data[1]) << 16) |
          (((unsigned int)data[2]) << 8) | (unsigned int)data[3];
      // unsigned Trigger_word1 = (((unsigned int)data[i + 3]) << 24) |
      // (((unsigned int)data[i + 2]) << 16) | (((unsigned int)data[i + 1]) <<
      // 8) | (unsigned int)data[i];
    ...
    bool getHitData(unsigned int &Word, bool second_hit, unsigned int &Col,
                    unsigned int &Row, unsigned int &ToT) const {
      // printf("getHitData word=%08x col_masked=%08x
      // %d\n",Word,PYBAR_DATA_RECORD_COLUMN_MASK &
      // Word,PYBAR_DATA_RECORD_MACRO(Word));
      if (!PYBAR_DATA_RECORD_MACRO(Word))
        return false; // No Data Record
    ...
    StandardPlane ConvertPlane(const std::vector<unsigned char> &data,
      ...
      plane.SetSizeZS(CHIP_MAX_COL_NORM + 1, CHIP_MAX_ROW_NORM + 1, 0,
                      consecutive_lvl1, StandardPlane::FLAG_DIFFCOORDS |
      ...
      for (unsigned int i = 4; i < data.size(); i += 4) {
        unsigned int Word = (((unsigned int)data[i]) << 24) |
                            (((unsigned int)data[i + 1]) << 16) |
                            (((unsigned int)data[i + 2]) << 8) |
                            (unsigned int)data[i + 3];
        // printf("ConvertPlane() word=%08x masked=%08x
        // %d\n",Word,(PYBAR_DATA_HEADER_MASK &
        // Word),PYBAR_DATA_HEADER_MACRO(Word));
        if (PYBAR_DATA_HEADER_MACRO(Word)) {
          lvl1++;
        } else {
          // First Hit
          if (getHitData(Word, false, Col, Row, ToT)) {
            // printf("ConvertPlane() First Hit %d, %d %d
            // %d\n",Col,Row,ToT,lvl1);
          ...
          if (getHitData(Word, true, Col, Row, ToT)) {
            // printf("ConvertPlane() Second Hit %08x, %d, %d %d
  ...
  class PyBARConverterPlugin : public DataConverterPlugin,
                               public PyBARConverterBase {
  ...
  public:
    ...
    virtual bool GetLCIOSubEvent(lcio::LCEvent &lcioEvent,
      ...
      // std::cout << "getlciosubevent (I4) event " <<
      // eudaqEvent.GetEventNumber() << " | " << GetTriggerID(eudaqEvent) <<
      // std::endl;
      ...
      const RawDataEvent &ev_raw =
          dynamic_cast<const RawDataEvent &>(eudaqEvent);

      std::vector<eutelescope::EUTelSetupDescription *> setupDescription;

      for (size_t chip = 0; chip < ev_raw.NumBlocks(); ++chip) {
        const std::vector<unsigned char> &buffer =
            dynamic_cast<const std::vector<unsigned char> &>(
                ev_raw.GetBlock(chip));
        ...
        // it helps to decode (and later to decode) parameters of all hits (x,
        // y, charge, ...) to
        // a single TrackerData object (zsFrame) that will correspond to a
        // single sensor in one event
        ...
        for (unsigned int i = 4; i < buffer.size(); i += 4) {
          unsigned int Word = (((unsigned int)buffer[i]) << 24) |
          ...
          if (PYBAR_DATA_HEADER_MACRO(Word)) {
            ...
            if (getHitData(Word, false, Col, Row, ToT)) {
              eutelescope::EUTelGenericSparsePixel *thisHit =
                  new eutelescope::EUTelGenericSparsePixel(Col, Row, ToT,
                                                           lvl1 - 1);
  ...
  private:
    ...
    PyBARConverterPlugin() : DataConverterPlugin(EVENT_TYPE) {}
