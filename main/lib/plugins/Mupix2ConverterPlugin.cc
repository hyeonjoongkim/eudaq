...
namespace eudaq {
  ...
  public:
    virtual void Initialize(const eudaq::Event &,
  ...
  private:
    ...
    void ExtractHit(const unsigned char *, unsigned, unsigned &,
                    unsigned &) const;
  ...
  bool Mupix2ConverterPlugin::GetStandardSubEvent(StandardEvent &dest,
    ...
    if (data == NULL) {
      ...
      StandardPlane plane(MUPIX2_SENSOR_ID, MUPIX2_EVENT_TYPE,
                          MUPIX2_SENSOR_TYPE);
  ...
  bool
  Mupix2ConverterPlugin::GetLCIOSubEvent(lcio::LCEvent &dest,
    ...
    CellIDEncoder<TrackerDataImpl> cell_encoder(
        EUTELESCOPE::ZSDATADEFAULTENCODING, collection);
    ...
    pixels.reset(new EUTelTrackerDataInterfacerImpl<EUTelGenericSparsePixel>(
        frame.get()));
  ...
  const unsigned char *
  Mupix2ConverterPlugin::GetRawDataChecked(const Event &ev) const {
    const RawDataEvent &raw = dynamic_cast<const RawDataEvent &>(ev);

    if (raw.NumBlocks() != 1) {
      std::cerr << "event " << raw.GetEventNumber() << ": "
                << "invalid number of data blocks" << std::endl;
      return NULL;
    }
    ...
    if (actual_size < 8) {
      std::cerr << "event " << raw.GetEventNumber() << ": "
                << "data block is too small" << std::endl;
      return NULL;
    }
  ...
  unsigned
  Mupix2ConverterPlugin::ExtractNumberOfHits(const unsigned char *data) const {
    return eudaq::getlittleendian<uint16_t>(data + 2);
  }

  // extract the trigger id from the raw data
  unsigned
  Mupix2ConverterPlugin::ExtractTriggerId(const unsigned char *data) const {
    return eudaq::getlittleendian<uint32_t>(data + 4);
  }

  // extract a single hit from the raw data
  void Mupix2ConverterPlugin::ExtractHit(const unsigned char *data,
                                         unsigned index, unsigned &col,
                                         unsigned &row) const {
