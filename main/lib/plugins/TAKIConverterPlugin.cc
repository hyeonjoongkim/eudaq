...
namespace eudaq {
...
#if USE_LCIO && USE_EUTELESCOPE
  void TAKIConverterPlugin::ConvertLCIOHeader(
      lcio::LCRunHeader &header, eudaq::Event const & /*bore*/,
      eudaq::Configuration const & /*conf*/) const {
    eutelescope::EUTelRunHeaderImpl runHeader(&header);
  }
  bool TAKIConverterPlugin::GetLCIOSubEvent(lcio::LCEvent &result,
    ...
    if (israw) {
      ...
      for (size_t i = 0; i < ev.NumBlocks(); ++i) {
        ...
        std::string mode =
            "RAW2"; // mode correct? will this be used somewhere????
        ...
        const short bias =
            64; // pedestal bias used during the cern summer testbeam 2009
      ...
      if (result.getEventNumber() == 0) {
        ...
        try {
          takiSetupCollection =
              static_cast<LCCollectionVec *>(result.getCollection("takiSetup"));
  ...
  bool TAKIConverterPlugin::GetStandardSubEvent(StandardEvent &result,
                                                const Event &source) const {
    if (source.IsBORE()) {
      // shouldn't happen
      return true;
    ...
    // If we get here it must be a data event
    const RawDataEvent &ev = dynamic_cast<const RawDataEvent &>(source);
    // printf(" [TAKIConvPlg::GetStdSubEv()]: TLU_eventnumber (as hex) from tag:
  ...
  StandardPlane
  TAKIConverterPlugin::ConvertPlane(const std::vector<unsigned char> &data,
                                    unsigned id) const {
    StandardPlane plane(id, "Taki", "Taki");

    unsigned tlu_eventnumber = ((((uint32_t)data[5])) & 0x000000FF) |
                               ((((uint32_t)data[4]) << 8) & 0x0000FF00) |
                               ((((uint32_t)data[3]) << 16) & 0x00FF0000) |
                               ((((uint32_t)data[2]) << 24) & 0xFF000000);
    /*
       tlu_eventnumber =     (( ((unsigned) *(dataPtr+5))       ) & 0x000000FF)
       //!only unsigned int available instead of ULL or UL!
       | (( ((unsigned) *(dataPtr+4)) << 8  ) & 0x0000FF00);
     */

    //! printf("[TAKIConverterPlugin::ConvertPlane()]: tlu_eventnumber (as hex)
    //out of stream: %lx\n", tlu_eventnumber);
    ...
    // printf("RECEIVED ONE EVENT: TID=%u, numPixels=%d (judging from content
    ...
    if (nDataBytesFromStreamContent != nDataBytesFromStreamHeader)
      printf("!!! number of data bytes from the stream contents = %d\n, number "
             "of data bytes from the stream header = %d\n\n !!!",
             nDataBytesFromStreamContent, nDataBytesFromStreamHeader);
    // else
    //  printf("!!! nDataBytesFromStreamContent matches
    //  nDataBytesFromStreamHeader (%d)!!!",nDataBytesFromStreamContent);

    plane.SetSizeZS(xsize, ysize,
                    (nDataBytesFromStreamContent /
                     3)); // Set the size for one frames of ... * ...

    // use data in order to decode your sensor
    const unsigned char *dataPtr = &data[6]; // first six bytes contain length
                                             // of incoming data (2 bytes) and
                                             // TID (4 bytes)
    for (unsigned int i = 0;
         i < (unsigned int)(nDataBytesFromStreamContent / 3); i++) {
