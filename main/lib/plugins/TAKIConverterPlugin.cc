...
bool TAKIConverterPlugin::GetLCIOSubEvent(lcio::LCEvent &result,
  ...
  if (israw) {
    ...
    for (size_t i = 0; i < ev.NumBlocks(); ++i) {
      ...
      std::string mode = "RAW2"; // mode correct? will this be used
                                 // somewhere????
...
TAKIConverterPlugin::ConvertPlane(const std::vector<unsigned char> &data,
  ...
  // else
  //  printf("!!! nDataBytesFromStreamContent matches nDataBytesFromStreamHeader
  //  (%d)!!!",nDataBytesFromStreamContent);
  ...
  const unsigned char *dataPtr = &data[6]; // first six bytes contain length of
                                           // incoming data (2 bytes) and TID (4
                                           // bytes)
  for (unsigned int i = 0; i < (unsigned int)(nDataBytesFromStreamContent / 3);
       i++) {
