...
EUDRBConverterBase::ConvertZS2(StandardPlane &plane,
  ...
  if (dbg)
    std::cout << "BaseAddress = 0x" << to_hex((word & 0xff000000) | 0x00400000)
              << std::endl;
  ...
  try {
    for (int frame = 1; frame <= 2; ++frame) {
      ...
      for (size_t i = 0; i < vec.size(); ++i) {
        ...
        if (dbg)
          std::cout << "Hit line " << (over ? "* " : ". ") << row << ", states "
                    << numstates << ":";
  ...
  if (dbg)
    std::cout << "TLUEventNumber = " << hexdec(word >> 8 & 0xffff, 0)
              << std::endl;
  if (dbg)
    std::cout << "NumFramesAtTrigger = " << hexdec(word & 0xff, 0) << std::endl;
  word = GET(++offset);
  if (dbg)
    std::cout << "EventWordCount = " << hexdec(word & 0x7ffff, 0) << std::endl;
...
void EUDRBConverterBase::ConvertRaw(StandardPlane &plane,
  ...
  plane.SetSizeRaw(info.Sensor().width, info.Sensor().height, info.Frames(),
                   StandardPlane::FLAG_WITHPIVOT | StandardPlane::FLAG_NEEDCDS |
...
void EUDRBConverterBase::ConvertLCIOHeader(
  ...
  std::vector<int> xMin(numplanes, 0), xMax(numplanes, 255), yMin(numplanes, 0),
      yMax(numplanes, 255);
...
bool EUDRBConverterBase::ConvertLCIO(lcio::LCEvent &result,
  ...
  for (size_t iPlane = 0; iPlane < numplanes; ++iPlane) {
    ...
    StandardPlane plane =
        ConvertPlane(GetPlane(source, iPlane), GetID(source, iPlane), tmp_evt);
    ...
    if (plane.GetFlags(StandardPlane::FLAG_ZS)) {
      ...
      for (size_t y = 0; y < currentDetector->getYNoOfPixel(); ++y) {
        ...
        // marker column
        currentCDSPos = copy(cdsBegin + offset, cdsBegin + (*(marker) + offset),
                             currentCDSPos);
