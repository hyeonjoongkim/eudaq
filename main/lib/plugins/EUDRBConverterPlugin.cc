...
namespace eudaq {
  ...
  void map_1x1(unsigned &x, unsigned &y, unsigned c, unsigned r, unsigned,
               unsigned, unsigned) {
    x = c;
    y = r;
  }

  void map_4x1(unsigned &x, unsigned &y, unsigned c, unsigned r, unsigned m,
  ...
  void map_2x2(unsigned &x, unsigned &y, unsigned c, unsigned r, unsigned m,
  ...
  struct SensorInfo {
    typedef void (*mapfunc_t)(unsigned &x, unsigned &y, unsigned c, unsigned r,
                              unsigned m, unsigned nc, unsigned nr);
    SensorInfo(const std::string &name, unsigned c, unsigned r, unsigned m,
               unsigned w, unsigned h, mapfunc_t mfunc = 0)
        : name(name), cols(c), rows(r), mats(m), width(w), height(h),
          mapfunc(mfunc) {}
    std::string name;
  ...
  struct BoardInfo {
    ...
    BoardInfo(const Event &ev, int brd)
      ...
      if (m_det == DET_NONE)
        EUDAQ_THROW("Unknown detector in EUDRBConverterPlugin: " + det);

      std::string mode = ev.GetTag("MODE" + to_string(brd));
      if (mode == "")
        mode = ev.GetTag("MODE", "RAW3");
      ...
      if (mode == "ZS")
        ...
        EUDAQ_THROW("Unknown mode in EUDRBConverterPlugin: " + mode);
    ...
    const SensorInfo &Sensor() const { return g_sensors[m_det]; }
    unsigned Frames() const { return m_mode > 0 ? m_mode : 1; }
    int m_version;
    E_DET m_det;
    E_MODE m_mode;
  };
  ...
  public:
    ...
    const BoardInfo &GetInfo(unsigned id) const {
      if (id >= m_info.size() || m_info[id].m_version < 1)
        EUDAQ_THROW("Unrecognised ID (" + to_string(id) + ", num=" +
                    to_string(m_info.size()) + ") converting EUDRB event");
      return m_info[id];
    }
    ...
    void ConvertLCIOHeader(lcio::LCRunHeader &header, eudaq::Event const &bore,
                           eudaq::Configuration const &conf) const;
    bool ConvertStandard(StandardEvent &stdEvent,
                         const Event &eudaqEvent) const;
    StandardPlane ConvertPlane(const std::vector<unsigned char> &data,
                               unsigned id, StandardEvent &evt) const {
      ...
      if (info.m_mode == BoardInfo::MODE_ZS2) {
        ...
        if (numoverflows)
          evt.SetTag("OVF" + to_string(id), numoverflows);
    ...
    static unsigned ConvertZS2(StandardPlane &plane,
                               const std::vector<unsigned char> &alldata,
                               const BoardInfo &info);
    static void ConvertZS(StandardPlane &plane,
                          const std::vector<unsigned char> &alldata,
                          const BoardInfo &info);
    static void ConvertRaw(StandardPlane &plane,
  ...
  protected:
    static size_t NumPlanes(const Event &event) {
      if (const RawDataEvent *ev = dynamic_cast<const RawDataEvent *>(&event)) {
        return ev->NumBlocks();
      } else if (const EUDRBEvent *ev =
                     dynamic_cast<const EUDRBEvent *>(&event)) {
        return ev->NumBoards();
      }
      return 0;
    }
  ...
  public:
    ...
    virtual unsigned GetTriggerID(Event const &ev) const {
      ...
      if (rawev.NumBlocks() < 1)
        return (unsigned)-1;
      const std::vector<unsigned char> &data =
          rawev.GetBlock(rawev.NumBlocks() - 1);
  ...
  class LegacyEUDRBConverterPlugin : public DataConverterPlugin,
                                     public EUDRBConverterBase {
    virtual void Initialize(const eudaq::Event &e,
                            const eudaq::Configuration &c) {
      FillInfo(e, c);
    }
  ...
  bool EUDRBConverterBase::ConvertStandard(StandardEvent &result,
                                           const Event &source) const {
    if (source.IsBORE()) {
      // shouldn't happen
      return true;
    ...
    for (size_t i = 0; i < numplanes; ++i) {
      result.AddPlane(
          ConvertPlane(GetPlane(source, i), GetID(source, i), result));
    }
    return true;
  }
...
#define GET(o) getbigendian<unsigned>(&alldata[(o)*4])
  unsigned
  EUDRBConverterBase::ConvertZS2(StandardPlane &plane,
                                 const std::vector<unsigned char> &alldata,
                                 const BoardInfo &info) {
    static const bool dbg = false;
    static const bool dbg2 = false;
    if (dbg)
      std::cout << "DataSize = " << hexdec(alldata.size(), 0) << std::endl;
    if (alldata.size() < 64)
      EUDAQ_THROW("Bad data packet (only " + to_string(alldata.size()) +
                  " bytes)");
    unsigned offset = 0, word = GET(offset);
    if (dbg)
      std::cout << "BaseAddress = 0x"
                << to_hex((word & 0xff000000) | 0x00400000) << std::endl;
    unsigned wordcount = word & 0xffffff;
    if (dbg)
      std::cout << "WordCount = " << hexdec(wordcount, 0) << std::endl;
    if (wordcount * 4 + 16 != alldata.size())
      EUDAQ_THROW("Bad wordcount (" + to_string(wordcount) + ", bytes=" +
                  to_string(alldata.size()) + ")");
    word = GET(offset = 1);
    if (dbg)
      std::cout << "Unknown = " << hexdec(word >> 8 & 0xffff) << std::endl;
    // offset 2 is a repeat of offset 0
    word = GET(offset = 3);
    unsigned sof = word;
    if (dbg)
      std::cout << "StartOfFrame = " << hexdec(sof, 0) << std::endl;
    word = GET(offset = 4);
    if (dbg)
      std::cout << "LocalEventNumber = " << hexdec(word >> 8 & 0xffff, 0)
                << std::endl;
    if (dbg)
      std::cout << "FrameNumberAtTrigger = " << hexdec(word & 0xff, 0)
                << std::endl;
    word = GET(offset = 5);
    unsigned pixadd = word & 0x3ffff;
    plane.SetPivotPixel((9216 + pixadd - sof + 56) % 9216);
    if (dbg)
      std::cout << "PixelAddressAtTrigger = " << hexdec(pixadd, 0)
                << ": pivot = " << hexdec(plane.PivotPixel(), 0) << std::endl;
    unsigned wordremain = wordcount - 12;
    unsigned totaloverflows = 0;
    plane.SetSizeZS(info.Sensor().width, info.Sensor().height, 0, 2,
    ...
    try {
      for (int frame = 1; frame <= 2; ++frame) {
        ...
        word = GET(offset += 2);
        if (dbg)
          std::cout << "M26FrameCounter_" << frame << " = " << hexdec(word, 0)
                    << std::endl;
        word = GET(++offset);
        unsigned count = word & 0xffff;
        if (dbg)
          std::cout << "M26WordCount_" << frame << " = " << hexdec(count, 0)
                    << ", " << hexdec(word >> 16, 0) << std::endl;
        if (count > wordremain)
          EUDAQ_THROW("Bad M26 word count (" + to_string(count) + ", remain=" +
                      to_string(wordremain) + ", total=" +
                      to_string(wordcount) + ")");
        wordremain -= count;
        ...
        for (size_t i = 0; i < vec.size(); ++i) {
          ...
          if (over) {
            numoverflows++;
            if (dbg2)
              std::cout << "*** Overflow in plane " << plane.ID() << ", row "
                        << row << std::endl;
          }
        }
        if (dbg)
          std::cout << "Frame pixels = " << npixels << std::endl;
        if (dbg)
          std::cout << "Frame overflows = " << numoverflows << std::endl;
        ++offset;
        totaloverflows += numoverflows;
      }
      if (dbg && totaloverflows)
        std::cout << "Total overflows = " << totaloverflows << std::endl;
    ...
    if (dbg)
      std::cout << "TLUEventNumber = " << hexdec(word >> 8 & 0xffff, 0)
                << std::endl;
    if (dbg)
      std::cout << "NumFramesAtTrigger = " << hexdec(word & 0xff, 0)
                << std::endl;
    word = GET(++offset);
    if (dbg)
      std::cout << "EventWordCount = " << hexdec(word & 0x7ffff, 0)
                << std::endl;
...
#undef GET
  ...
  void EUDRBConverterBase::ConvertZS(StandardPlane &plane,
                                     const std::vector<unsigned char> &alldata,
                                     const BoardInfo &info) {
    ...
    unsigned npixels =
        (alldata.size() - headersize - trailersize) / 4 - padding;
    ...
    for (unsigned i = 0; i < npixels; ++i) {
      ...
      unsigned x, y;
      info.Sensor().mapfunc(x, y, col, row, mat, info.Sensor().cols,
                            info.Sensor().rows);
  ...
  void EUDRBConverterBase::ConvertRaw(StandardPlane &plane,
                                      const std::vector<unsigned char> &data,
                                      const BoardInfo &info) {
    unsigned headersize = 8, trailersize = 8;
    if (info.m_version > 2) {
    ...
    plane.SetPivotPixel(((data[5] & 0x3) << 16) | (data[6] << 8) | data[7]);
    unsigned possible1 = 2 * info.Sensor().cols * info.Sensor().rows *
                         info.Sensor().mats * info.Frames();
    unsigned possible2 = 2 * (info.Sensor().cols * info.Sensor().rows - 1) *
                         info.Sensor().mats * info.Frames();
    ...
    if (data.size() - headersize - trailersize == possible1) {
      ...
      EUDAQ_THROW("Bad raw data size (" +
                  to_string(data.size() - headersize - trailersize) +
                  ") expecting " + to_string(possible1) + " or " +
                  to_string(possible2));
    }
    // unsigned npixels = info.Sensor().cols * info.Sensor().rows *
    // info.Sensor().mats;
    plane.SetSizeRaw(info.Sensor().width, info.Sensor().height, info.Frames(),
                     StandardPlane::FLAG_WITHPIVOT |
                         StandardPlane::FLAG_NEEDCDS |
                         StandardPlane::FLAG_NEGATIVE);
    // plane.m_mat.resize(plane.m_pix[0].size());
    ...
    for (unsigned row = 0; row < info.Sensor().rows; ++row) {
      for (unsigned col = 0; col < info.Sensor().cols; ++col) {
        if (missingpixel && row == info.Sensor().rows - 1 &&
            col == info.Sensor().cols - 1)
        ...
        for (size_t frame = 0; frame < info.Frames(); ++frame) {
          for (size_t mat = 0; mat < info.Sensor().mats; ++mat) {
            ...
            bool pivot = (info.m_version < 2)
                             ? (row << 7 | col) >= plane.PivotPixel()
                             : (row << 9 | col) >= plane.PivotPixel();
  ...
  void EUDRBConverterBase::ConvertLCIOHeader(
    ...
    std::vector<int> xMin(numplanes, 0), xMax(numplanes, 255),
        yMin(numplanes, 0), yMax(numplanes, 255);
  ...
  bool EUDRBConverterBase::ConvertLCIO(lcio::LCEvent &result,
                                       const Event &source) const {
    ...
    // const RawDataEvent & rawDataEvent = dynamic_cast< const RawDataEvent & >
    // ( source ) ;
    ...
    for (size_t iPlane = 0; iPlane < numplanes; ++iPlane) {
      ...
      StandardPlane plane = ConvertPlane(GetPlane(source, iPlane),
                                         GetID(source, iPlane), tmp_evt);
