...
#if USE_LCIO && USE_EUTELESCOPE
  void
  APIXCTConverterPlugin::ConvertLCIOHeader(lcio::LCRunHeader &header,
                                           eudaq::Event const &,
  ...
  bool APIXCTConverterPlugin::GetLCIOSubEvent(lcio::LCEvent &lcioEvent,
    ...
    try {
      zsDataCollection = static_cast<LCCollectionVec *>(
          lcioEvent.getCollection("zsdata_apix"));
          ...
          if (m_nFeSensor[m_feToSensorid[hits[i].link]] == 4) { // 4-chip module
            ...
            /*
                        ...
                        if (row==329){
                          row = 336;
                          col=col;
                          eutelescope::EUTelGenericSparsePixel *thisHitgang1 =
               new eutelescope::EUTelGenericSparsePixel( row, col, ToT,  lvl1);
                          sparseFrame->addSparsePixel( thisHitgang1 );
                          tmphits.push_back( thisHitgang1 );
                          row = 340;
                          col=col;
                          eutelescope::EUTelGenericSparsePixel *thisHitgang2 =
               new eutelescope::EUTelGenericSparsePixel( row, col, ToT,  lvl1);
    ...
    if (lcioEvent.getEventNumber() == 0) {
      ...
      try {
        apixSetupCollection = static_cast<LCCollectionVec *>(
            lcioEvent.getCollection("apix_setup"));
