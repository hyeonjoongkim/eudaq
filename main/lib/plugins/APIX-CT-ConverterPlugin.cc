...
public:
  ...
  FormattedRecord(FormattedRecord::type ty) { m_record.ui = ty; }
  FormattedRecord(unsigned &wd) { m_record.ui = wd; }
...
private:
  ...
  void ConvertPlanes(const RawDataEvent &event,
                     std::map<int, StandardPlane> &result) const;
  APIXCTConverterPlugin()
      : DataConverterPlugin("APIX-CT"), m_nFrames(1),
        m_feToSensorid(*new std::map<int, int>),
        m_fepos(*new std::map<int, int>), m_sensorids(*new std::vector<int>),
...
#if USE_LCIO && USE_EUTELESCOPE
void APIXCTConverterPlugin::ConvertLCIOHeader(
    lcio::LCRunHeader &header, eudaq::Event const &,
    eudaq::Configuration const &) const {
  eutelescope::EUTelRunHeaderImpl runHeader(&header);
}

bool APIXCTConverterPlugin::GetLCIOSubEvent(lcio::LCEvent &lcioEvent,
  ...
  lcioEvent.parameters().setValue(eutelescope::EUTELESCOPE::EVENTTYPE,
                                  eutelescope::kDE);
  ...
  try {
    zsDataCollection =
        static_cast<LCCollectionVec *>(lcioEvent.getCollection("zsdata_apix"));
  ...
  for (size_t sensor = 0; sensor < m_sensorids.size(); sensor++) {
    if (lcioEvent.getEventNumber() == 0) {
      eutelescope::EUTelPixelDetector *currentDetector =
          new eutelescope::EUTelAPIXMCDetector(2);
    ...
    // it helps to decode (and later to decode) parameters of all hits (x, y,
    // charge, ...) to
    // a single TrackerData object (zsFrame) that will correspond to a single
    // sensor in one event
    ...
    for (size_t i = 0; i < hits.size(); i++) {
      if (m_feToSensorid[hits[i].link] == m_sensorids[sensor]) {
        if (m_nFeSensor[m_feToSensorid[hits[i].link]] == 4) { // 4-chip module
          ...
          int lvl1 = hits[i].lv1;
          int ToT = hits[i].tot;
          eutelescope::EUTelGenericSparsePixel *thisHit =
              new eutelescope::EUTelGenericSparsePixel(col, row, ToT, lvl1);
          ...
          /*
                      ...
                      if (row==329){
                        row = 336;
                        col=col;
                        eutelescope::EUTelGenericSparsePixel *thisHitgang1 = new
             eutelescope::EUTelGenericSparsePixel( row, col, ToT,  lvl1);
                        sparseFrame->addSparsePixel( thisHitgang1 );
                        tmphits.push_back( thisHitgang1 );
                        row = 340;
                        col=col;
                        eutelescope::EUTelGenericSparsePixel *thisHitgang2 = new
             eutelescope::EUTelGenericSparsePixel( row, col, ToT,  lvl1);
                      ...
                      if( m_sensorids[sensor] + chip_id_offset == 21)
                        {
                          ...
                          int col = ( (rem4r<2 && rem2c==1) || (rem4r>1 &&
             rem2c==0) ) ? Col1*2 : Col1*2+1;
                          int row = (int)(Row1/2);
                          eutelescope::EUTelGenericSparsePixel *thisHit = new
             eutelescope::EUTelGenericSparsePixel( col, row, hits[i].tot,
             hits[i].lv1);
                          sparseFrame->addSparsePixel( thisHit );
                          tmphits.push_back( thisHit );
                        }
          ...
          */
          {
            ...
            int row = hits[i].row;
            eutelescope::EUTelGenericSparsePixel *thisHit =
                new eutelescope::EUTelGenericSparsePixel(col, row, hits[i].tot,
                                                         hits[i].lv1);
            sparseFrame->addSparsePixel(thisHit);
            tmphits.push_back(thisHit);
          }
          // int col=(1+m_fepos[hits[i].link])*NCOL-1-hits[i].col; //left or
          // right on 2-chip module
          // eutelescope::EUTelGenericSparsePixel *thisHit = new
          // eutelescope::EUTelGenericSparsePixel( col, row, hits[i].tot,
          // hits[i].lv1);
          // sparseFrame->addSparsePixel( thisHit );
    ...
    // write TrackerData object that contains info from one sensor to LCIO
    // collection
    zsDataCollection->push_back(zsFrame.release());

    // clean up
    for (std::list<eutelescope::EUTelGenericSparsePixel *>::iterator it =
             tmphits.begin();
         it != tmphits.end(); it++) {
      delete (*it);
    }
  ...
  if (lcioEvent.getEventNumber() == 0) {
    ...
    try {
      apixSetupCollection =
          static_cast<LCCollectionVec *>(lcioEvent.getCollection("apix_setup"));
 ...
 * I think this is a good idea, as they will be act as independent detectors in
 * the tracking
 */
#endif
...
void APIXCTConverterPlugin::Initialize(const Event &source,
                                       const Configuration &) {
  ...
  std::cout << nFrontends << " frontends and " << m_sensorids.size()
            << " sensors." << std::endl;
  // std::cout << " Nrows , NColumns = " << m_NumRows << "  ,  " <<
  // m_NumColumns << std::endl;
  // std::cout << " Initial row , column = " << m_InitialRow << "  ,  " <<
  // m_InitialColumn << std::endl;
}

bool APIXCTConverterPlugin::GetStandardSubEvent(StandardEvent &result,
  ...
  unsigned eudetTrig =
      getlittleendian<unsigned int>(&ev.GetBlock(0)[36]) & 0x7fff;
  ...
  for (size_t i = 0; i < m_sensorids.size(); i++) {
    ...
    int colmult = 1;
    int rowmult = 1;
    if (m_nFeSensor[sensorid] > 1)
      colmult = 2; // 2 or 4 chip module
    if (m_nFeSensor[sensorid] == 4)
      rowmult = 2; // 4 chip module
    if (rowmult > 1)
      plane.SetSizeZS(rowmult * NROW, colmult * NCOL, 0, m_nFrames,
                      StandardPlane::FLAG_DIFFCOORDS |
                          StandardPlane::FLAG_ACCUMULATE);
    else
      plane.SetSizeZS(colmult * NCOL, NROW, 0, m_nFrames,
                      StandardPlane::FLAG_DIFFCOORDS |
                          StandardPlane::FLAG_ACCUMULATE);
...
unsigned APIXCTConverterPlugin::GetTriggerID(eudaq::Event const &ev) const {
  ...
  if (rev.NumBlocks() < 1)
    return (unsigned)-1;
  unsigned eudetTrig =
      getlittleendian<unsigned int>(&rev.GetBlock(0)[36]) & 0x7fff;
  return eudetTrig;
}

void APIXCTConverterPlugin::ConvertPlanes(
    const RawDataEvent &ev, std::map<int, StandardPlane> &result) const {
  ...
  for (unsigned int i = 0; i < hits.size(); i++) {
    ...
    if (result.find(sid) == result.end()) {
      std::cerr << "APIX-MC-ConvertPlugin::ConvertPlanes: Bad index: " << sid
                << std::endl;
      ...
      result[sid].PushPixel(NCOL * (m_fepos[hit.link] + 1) - 1 - hit.col,
                            hit.row, hit.tot, false, hit.lv1);
    }
  }
}

std::vector<CTELHit>
APIXCTConverterPlugin::decodeData(const RawDataEvent &ev) const {
  ...
  unsigned triggerword =
      (getlittleendian<unsigned int>(&block0[36]) & 0xff0000) >> 16;
  unsigned hitbusword =
      (getlittleendian<unsigned int>(&block0[36]) & 0xf000000) >> 24;
  ...
  while (indx < pixblock.size()) {
    ...
    if (current.isHeader()) {
      ntrg++; // really, this is the total number of triggers summed over all
              // the modules
      link = current.getLink();
      l1id = current.getL1id();
      bxid = current.getBxid() & 0xff;
      // std::cout<<"l1id "<<l1id<<" ; bxid "<<bxid<<" ; link
      // "<<link<<std::endl;
      ...
      std::cout << "ERROR in CTELConverterPlugin:  invalid data block. "
                << std::endl;
      break;
    }
    indx += 4;
