...
namespace eudaq {
  ...
  public:
      ...
      if (const RawDataEvent *rev = dynamic_cast<const RawDataEvent *>(&ev)) {
        if (rev->NumBlocks() > 0 && rev->GetBlock(0).size() >= 4) {
          std::vector<unsigned char> data8 =
              rev->GetBlock(0); // block 0 is trigger data
    ...
    virtual bool GetStandardSubEvent(StandardEvent &sev,
      ...
      std::vector<unsigned char> data =
          rev->GetBlock(1); // block 1 is pixel data
      ...
      for (size_t i = 0; i < ZSDataX.size(); ++i) {
        ...
        std::cout << "[DATA] "
                  << " " << (int)ZSDataX[i] << " " << (int)ZSDataY[i] << " "
                  << (int)ZSDataTOT[i] << " " << (int)ZSDataTOA[i] << std::endl;
        std::cout << "!CID " << tid << " " << (int)ZSDataX[i] << " "
                  << (int)ZSDataY[i] << std::endl;
      }
  ...
  private:
    CLICpixConverterPlugin() : DataConverterPlugin(EVENT_TYPE) {}
