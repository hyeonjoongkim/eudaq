...
namespace eudaq {
  ...
  private:
    StandardPlane ConvertPlane(const std::vector<unsigned char> &data,
                               unsigned id) const;
    FORTISConverterPlugin()
        : DataConverterPlugin("FORTIS"), m_NumRows(FORTIS_ROWS),
          m_NumColumns(FORTIS_COLUMNS), m_InitialRow(0), m_InitialColumn(0) {}
...
#if USE_LCIO && USE_EUTELESCOPE
  void FORTISConverterPlugin::ConvertLCIOHeader(
      lcio::LCRunHeader &header, eudaq::Event const & /*bore*/,
      eudaq::Configuration const & /*conf*/) const {
    eutelescope::EUTelRunHeaderImpl runHeader(&header);
  }
  bool FORTISConverterPlugin::GetLCIOSubEvent(lcio::LCEvent &result,
                                              const Event &source) const {

    bool israw = true; // TODO: lets try to determine the event type once and
                       // then assume it.
    ...
    if (israw) {
      ...
      for (size_t i = 0; i < ev.NumBlocks(); ++i) {
        ...
        std::string mode =
            "ZS2"; // mode correct? will this be used somewhere????
        ...
        // const unsigned int frameNumber = 0; // for now just look at the first
        // frame out of two.
        ...
        for (unsigned int i = 0; i < hitpixel; ++i) {
          ...
          hardcore_matrix.at(x).at(y) =
              (short int)plane.GetPixel(i); //, frameNumber);
        }

        for (int yPixel = 0; yPixel <= currentDetector->getYMax(); yPixel++) {
          for (int xPixel = 0; xPixel <= currentDetector->getXMax(); xPixel++) {
            short amp = 0; // what would be the best default value? maybe 0
                           // because the pedestal and noise processor will then
                           // mask these pixels as bad pixels.
            try {
      ...
      if (result.getEventNumber() == 0) {
        ...
        try {
          fortisSetupCollection = static_cast<LCCollectionVec *>(
              result.getCollection("FortisSetup"));
          fortisSetupExists = true;
        } catch (...) {
          fortisSetupCollection =
  ...
  void FORTISConverterPlugin::Initialize(const Event &source,
                                         const Configuration &) {
    ...
    // std::cout << " FORTIS: Nrows , NColumns = " << m_NumRows << "  ,  " <<
    // m_NumColumns << std::endl;
    EUDAQ_EXTRA(std::string(" FORTIS: Nrows , NColumns = ") +
                to_string(m_NumRows) + "  ,  " + to_string(m_NumColumns) +
                ": ");
    // std::cout << " FORTIS: Initial row , column = " << m_InitialRow << "  ,
    // " <<  m_InitialColumn << std::endl;
    EUDAQ_EXTRA(std::string(" FORTIS: Initial row , column = ") +
                to_string(m_InitialRow) + "  ,  " + to_string(m_InitialColumn) +
                ": ");
  }

  bool FORTISConverterPlugin::GetStandardSubEvent(StandardEvent &result,
                                                  const Event &source) const {
#if FORTIS_DEBUG
    ...
    if (ev.NumBlocks() > 1) {
      ...
      // if we have the frame data, then take a copy so that events with only
      // pivot row info can refer to it
      m_frameDataBuffer = ConvertPlane(ev.GetBlock(0), ev.GetID(0));
      m_parentEvent = ev.GetEventNumber();
    } else if (m_parentEvent !=
               ev.GetTag("PARENTEVENT", 0U)) { // if we get here we have no
                                               // frame data, so check that
                                               // parent event is correct...
      EUDAQ_ERROR("FORTIS PARENT EVENT doesn't match .... ");
    }

    StandardPlane &localPlane =
        result.AddPlane(m_frameDataBuffer); // add plane to data from copy.

    unsigned pivotRow = ev.GetTag("PIVOTROW", 0);
    localPlane.SetPivotPixel(
        pivotRow); // set the pivot pixel to be the pivot row
  ...
  StandardPlane
  FORTISConverterPlugin::ConvertPlane(const std::vector<unsigned char> &data,
                                      unsigned id) const {
#if FORTIS_DEBUG
    ...
    if (data.size() < expected)
      EUDAQ_THROW("Bad Data Size (" + to_string(data.size()) + " < " +
                  to_string(expected) + ")");
    ...
    plane.SetSizeZS(FORTIS_ROWS, FORTIS_COLUMNS, npixels, 2,
                    StandardPlane::FLAG_WITHPIVOT); // Set the size for two
                                                    // frames of 512*512
    ...
    for (size_t Frame = 0; Frame < 2; ++Frame) { // Frame was <2
      ...
      std::cout << "FORTIS:: frame number in data = "
                << eudaq::to_hex(frame_number[Frame]) << std::endl;
#endif

      for (size_t Row = 0; Row < m_NumRows; ++Row) {

        unsigned int header_offset =
            ((m_NumColumns + sizeof(short)) * Row + Frame * nwords) *
            sizeof(short);

        unsigned short TriggerWord =
            getlittleendian<unsigned short>(&data[header_offset]);
        unsigned short TriggerCount = TriggerWord & 0x00FF;

#if FORTIS_DEBUG
        unsigned short LatchWord = getlittleendian<unsigned short>(
            &data[header_offset + sizeof(short)]);
        std::cout << "Row = " << Row
                  << " Header = " << eudaq::to_hex(TriggerWord) << "    "
                  << LatchWord << std::endl;
#endif
        if ((triggerRow == 0) && (TriggerCount > 0)) {
          triggerRow = Row;
        }

        for (size_t Column = 0; Column < m_NumColumns; ++Column) {
          unsigned offset =
              (Column + 2 + (m_NumColumns + 2) * Row + Frame * nwords) *
              sizeof(short);
          unsigned short d = getlittleendian<unsigned short>(&data[offset]);
          plane.SetPixel(i, Column + m_InitialColumn, Row + m_InitialRow, d,
                         (unsigned)Frame); // need to put option to have
                                           // inversion at this stage ( ie. put
                                           // in 0xFFFF -d rather than d. This
                                           // is because by selecting jumpers
                                           // can read out FORTIS with singled
                                           // ended *inverting* amplifier.
          ++i;
        }
      }
    }
    ...
    if (frame_number[1] != (frame_number[0] + 1)) {
      std::cout << "Frame_number[0] ,  Frame_number[1] = " << frame_number[0]
                << "   " << frame_number[1] << std::endl;

      EUDAQ_THROW(
          "FORTIS data corruption: Frame_number[1] != Frame_number[0]+1 (" +
          to_string(frame_number[1]) + " != " + to_string(frame_number[0]) +
          " +1 )");
    }
