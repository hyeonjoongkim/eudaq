...
bool FORTISConverterPlugin::GetLCIOSubEvent(lcio::LCEvent &result,
  ...
  bool israw = true; // TODO: lets try to determine the event type once and then
                     // assume it.
    ...
    if (result.getEventNumber() == 0) {
      ...
      try {
        fortisSetupCollection =
            static_cast<LCCollectionVec *>(result.getCollection("FortisSetup"));
...
bool FORTISConverterPlugin::GetStandardSubEvent(StandardEvent &result,
  ...
  if (ev.NumBlocks() > 1) {
    ...
    m_parentEvent = ev.GetEventNumber();
             ...
             ev.GetTag("PARENTEVENT", 0U)) { // if we get here we have no frame
                                             // data, so check that parent event
                                             // is correct...
...
FORTISConverterPlugin::ConvertPlane(const std::vector<unsigned char> &data,
  ...
  plane.SetSizeZS(
      FORTIS_ROWS, FORTIS_COLUMNS, npixels, 2,
      StandardPlane::FLAG_WITHPIVOT); // Set the size for two frames of 512*512
    ...
    for (size_t Row = 0; Row < m_NumRows; ++Row) {
...
#if FORTIS_DEBUG
      unsigned short LatchWord =
          getlittleendian<unsigned short>(&data[header_offset + sizeof(short)]);
      std::cout << "Row = " << Row << " Header = " << eudaq::to_hex(TriggerWord)
                << "    " << LatchWord << std::endl;
#endif
      ...
      for (size_t Column = 0; Column < m_NumColumns; ++Column) {
        ...
        plane.SetPixel(i, Column + m_InitialColumn, Row + m_InitialRow, d,
                       (unsigned)Frame); // need to put option to have inversion
                                         // at this stage ( ie. put in 0xFFFF -d
                                         // rather than d. This is because by
                                         // selecting jumpers can read out
                                         // FORTIS with singled ended
                                         // *inverting* amplifier.
        ++i;
      }
    }
