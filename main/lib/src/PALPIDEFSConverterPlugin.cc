...
public:
  ...
  bool DecodeAlpide2ChipHeader(const char Data, int ChipId) const {
    ...
    if (FoundChipId != (ChipId & 0xf)) {
      std::cout << "Error, found wrong chip ID " << FoundChipId << " in header."
                << std::endl;
      return false;
    }
    return true;
    ...
    while ((byte + 1 < data.size()) && (!ended)) {
      ...
      case DT_CHIPTRAILER:
        ...
        if (current_region < 31) {
          std::cout << "Error, chip trailer found before last region, current "
                       "region = " << current_region << std::endl;
          return false;
        }
        started = false;
        ended = true;
      ...
      case DT_REGHEADER:
        if (!started) {
          std::cout << "Error, region header found before chip header or after "
                       "chip trailer" << std::endl;
          return false;
        }
      ...
      case DT_DATASHORT:
        if (!started) {
          std::cout << "Error, hit data found before chip header or after chip "
                       "trailer, offending word = " << std::hex
  ...
  virtual bool GetStandardSubEvent(StandardEvent &sev, const Event &ev) const {
    ...
    if (m_nLayers < 0) {
      cout << "ERROR: Number of layers < 0 --> " << m_nLayers << ". Check BORE!"
           << endl;
      return false;
    }
...
// RAW dump
        ...
        while (pos + 1 < data.size()) { // always need 2 bytes left
          ...
          // For pAlpide1 DecodeChipData decodes 1 region, then returns to this
          // loop until region ==31
        ...
        for (int i = 0; i < m_nLayers - 1; i++) {
          if (timestamps[i + 1] == 0 ||
              (fabs(1.0 - (double)timestamps[i] / timestamps[i + 1]) > 0.0001 &&
...
///////////////////////////////////////////////////////////
  ...
  virtual bool GetLCIOSubEvent(lcio::LCEvent &lev,
    ...
    lev.parameters().setValue(
        "TIMESTAMP_H", (int)((sev.GetTimestamp() & 0xFFFFFFFF00000000) >> 32));
    ...
    try {
      zsDataCollection =
          static_cast<LCCollectionVec *>(lev.getCollection("zsdata_pALPIDEfs"));
...
protected:
  ...
  int ParseXML(std::string xml, int base, int rgn, int sub, int begin) {
    ...
    for (TiXmlElement *eBase = root->FirstChildElement("address"); eBase != 0;
      ...
      if (base != atoi(eBase->Attribute("base")))
        continue;
      for (TiXmlElement *eRgn = eBase->FirstChildElement("address"); eRgn != 0;
           eRgn = eRgn->NextSiblingElement("address")) {
        if (rgn != atoi(eRgn->Attribute("rgn")))
          continue;
        for (TiXmlElement *eSub = eRgn->FirstChildElement("address"); eSub != 0;
             eSub = eSub->NextSiblingElement("address")) {
          ...
          for (TiXmlElement *eBegin = eSub->FirstChildElement("value");
            ...
            return (int)strtol(
                eBegin->FirstChildElement("content")->FirstChild()->Value(), 0,
                16);
          }
          return -5;
        }
  ...
  bool analyse_threshold_scan(const unsigned char *const data,
                              ...
                              float **thr_rms, float **noise, float **noise_rms,
    ...
    *noise_rms = new float[n_sectors]; // used for the some of squares
    ...
    for (unsigned int i_pixel = 0; i_pixel < n_pixels; ++i_pixel) {
      if (data[i_pixel * n_points] != 255) {
        ...
        for (unsigned int i_point = 0; i_point < n_points; ++i_point) {
          y[i_point] =
              ((double)data[i_pixel * n_points + i_point]) / ((double)n_events);
