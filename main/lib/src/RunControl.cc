...
void RunControl::InitData(const ConnectionInfo &id) {
  ...
  if (m_runnumber >= 0) {
    ...
    if (m_runnumber != static_cast<int32_t>(thisRunNumber)) {
      EUDAQ_THROW("DataCollector run number mismatch! Previously received run "
                  "number does not match run number reported by newly "
  ...
  // found)
  std::vector<std::string> otherDC; // holds names of other DCs found (important
                                    // for announcing this DC)
  ...
  std::cout << "DataServer responded: full server address determined to be  = '"
