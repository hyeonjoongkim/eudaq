...
TCPServer::TCPServer(const std::string &param)
    : m_port(from_string(param, 44000)),
      m_srvsock(socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)), m_maxfd(m_srvsock) {
  if (m_srvsock == (SOCKET)-1)
...
TCPServer::~TCPServer() {
  for (size_t i = 0; i < m_conn.size(); ++i) {
    ConnectionInfoTCP *inf = dynamic_cast<ConnectionInfoTCP *>(m_conn[i].get());
...
#ifdef WIN32
    int result =
        select(static_cast<int>(m_sock + 1), &tempset, NULL, NULL, &timeremain);
#else
    SOCKET result =
        select(static_cast<int>(m_sock + 1), &tempset, NULL, NULL, &timeremain);
#endif
