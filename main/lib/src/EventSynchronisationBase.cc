...
SyncBase::SyncBase(bool sync)
    : m_registertProducer(0), m_ProducerEventQueue(0), NumberOfEventsToSync_(1),
      longTimeDiff_(0), isAsync_(false), m_TLUs_found(0), m_sync(sync) {}
    ...
    if (TLU_ID == BOREvent.GetEvent(i)->get_id()) {
      if (m_TLUs_found == 0) {
        ...
        m_ProducerId2Eventqueue[getUniqueID(fileIndex, i)] =
            id++; // only the first TLU gets threated differently all others are
                  // just producers
      }
      ++m_TLUs_found;
    } else {
...
void SyncBase::PrepareForEvents() {
  ...
  if (!m_sync) {
    ...
    if (m_TLUs_found == 0) {
      EUDAQ_THROW("no TLU events found in the data\n for the resynchronisation "
                  "it is nessasary to have a TLU in the data stream \n for now "
                  "the synchrounsation only works with the old TLU (date "
                  "12.2013)");
    } else if (m_TLUs_found > 1) {
      std::cout << "more than one TLU detected only the first TLU is used for "
