...
DataCollector::DataCollector(const std::string &name,
  ...
  m_thread =
      std::unique_ptr<std::thread>(new std::thread(DataCollector_thread, this));
...
void DataCollector::OnReceive(const ConnectionInfo &id,
  ...
  if (inf.events.back()->IsEORE())
    std::cout << "Received EORE Event from " << id << ": " << *ev << std::endl;
    ...
    for (size_t i = 0; i < m_buffer.size(); ++i) {
      ...
      if ((m_buffer[i].events.front()->GetEventNumber() != m_eventnumber) &&
        ...
        if (ev.GetEventNumber() % 1000 == 0) {
          // dhaas: added if-statement to filter out TLU event number 0, in case
          // of bad clocking out
...
void DataCollector::DataHandler(TransportEvent &ev) {
  ...
  case (TransportEvent::CONNECT):
    ...
    if (m_listening) {
      ...
      m_dataserver->SendPacket("ERROR EUDAQ DATA Not accepting new connections",
                               ev.id, true);
